...
数据源：market_data.db 表 MFI资金流量扫描器.py
字段：排名,币种,周期,方向,强度,MFI值,信号概述,当前价格,成交额（USDT）,数据时间
"""
...
class MFI排行卡片(RankingCard):
    FALLBACK = "card.mfi.fallback"
    ...
    async def handle_callback(self, update, context, services: Dict[str, object]) -> bool:
        ...
        if h is None:
            return False
        data = query.data or ""
        if data in (self.card_id, self.entry_callback, "mfi_ranking_refresh"):
            await self._reply(query, h, ensure)
            return True
        if data == "mfi_nop":
            return True
        if data.startswith("mfi_market_"):
            h.user_states["mfi_market"] = data.replace("mfi_market_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("mfi_sort_field_"):
            h.user_states["mfi_sort_field"] = data.replace("mfi_sort_field_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("mfi_period_"):
            h.user_states["mfi_period"] = data.replace("mfi_period_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("mfi_sort_"):
            h.user_states["mfi_sort"] = data.replace("mfi_sort_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("mfi_limit_"):
            val = data.replace("mfi_limit_", "")
            if val.isdigit():
                h.user_states["mfi_limit"] = int(val)
                await self._edit(query, h, ensure)
                return True
            return False
        if data.startswith("field_mfi_toggle_"):
            col = data.replace("field_mfi_toggle_", "")
            fields_state = self._ensure_field_state(h)
            if col in fields_state:
                fields_state[col] = not fields_state[col]
                h.user_states["mfi_fields"] = fields_state
            await self._edit(query, h, ensure)
            return True
        return False
    ...
    async def _build_payload(self, h, ensure, lang: str = "zh_CN", update=None) -> Tuple[str, object]:
        period = h.user_states.get("mfi_period", "15m")
        sort_order = h.user_states.get("mfi_sort", "desc")
        limit = h.user_states.get("mfi_limit", 10)
    ...
    def _build_keyboard(self, h):
        ...
        period = h.user_states.get("mfi_period", "15m")
        sort_order = h.user_states.get("mfi_sort", "desc")
        current_limit = h.user_states.get("mfi_limit", 10)
        current_sort_field = h.user_states.get("mfi_sort_field", "strength")
        market = h.user_states.get("mfi_market", "futures")
        ...
        if show_market_row:
            kb.append([
                b("现货", "mfi_market_spot", active=market == "spot"),
                b("期货", "mfi_market_futures", active=market == "futures"),
            ])
        ...
        for col_id, label, _ in self.general_display_fields:
            ...
            gen_row.append(InlineKeyboardButton(show_label, callback_data=f"field_mfi_toggle_{col_id}"))
        kb.append(gen_row)
        ...
        for col_id, label, _ in self.special_display_fields:
            state_on = fields_state.get(col_id, True)
            show_label = label if state_on else f"❎{label}"
            spec_row.append(InlineKeyboardButton(show_label, callback_data=f"field_mfi_toggle_{col_id}"))
        kb.append(spec_row)

        general_sort = [("quote_volume", "成交额"), ("振幅", "振幅"), ("成交笔数", "成交笔数"), ("主动买卖比", "主动买卖比"), ("price", "价格")]
        kb.append([
            b(lbl, f"mfi_sort_field_{fid}", active=(current_sort_field == fid))
            for fid, lbl in general_sort
        ])

        special_sort = [("mfi", "MFI")]
        kb.append([
            b(lbl, f"mfi_sort_field_{fid}", active=(current_sort_field == fid))
            for fid, lbl in special_sort
        ])
        periods = ["1m", "5m", "15m", "1h", "4h", "1d", "1w"]
        kb.append([b(p, f"mfi_period_{p}", active=p == period) for p in periods])

        kb.append([
            b("降序", "mfi_sort_desc", active=sort_order == "desc"),
            b("升序", "mfi_sort_asc", active=sort_order == "asc"),
            b("10条", "mfi_limit_10", active=current_limit == 10),
            b("20条", "mfi_limit_20", active=current_limit == 20),
            b("30条", "mfi_limit_30", active=current_limit == 30),
        ])

        kb.append([
            _btn_auto(None, "🏠主菜单", "ranking_menu"),
            _btn_auto(None, "🔄刷新", "mfi_ranking_refresh"),
        ])
    ...
    def _load_rows(self, period: str, sort_order: str, limit: int, sort_field: str, field_state: Dict[str, bool], lang: str | None = None) -> Tuple[List[List[str]], str]:
        items: List[Dict] = []
        try:
            metrics = self.provider.merge_with_base("MFI资金流量榜单", period, base_fields=["当前价格", "成交额"])
            for row in metrics:
                sym = format_symbol(row.get("symbol") or row.get("交易对") or row.get("币种") or "")
                if not sym:
                    continue
                mfi_val = float(row.get("MFI值") or 0)
                items.append({
                    "symbol": sym,
                    "strength": mfi_val,
                    "mfi": mfi_val,
            ...
            self._logger.warning("读取 MFI 榜单失败: %s", exc)
    ...
    def _ensure_field_state(self, h) -> Dict[str, bool]:
        state = h.user_states.get("mfi_fields")
        if not state:
            state = {}
            ...
            for _off in {"quote_volume", "振幅", "成交笔数", "主动买卖比"}:
                if _off in state:
                    state[_off] = False
            h.user_states["mfi_fields"] = state
