...
class FuturesTopSentimentCard(RankingCard):
    ...
    FALLBACK = "card.big_sentiment.fallback"
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            default_state={
                ...
                "top_sort": "desc",
                "top_limit": 10,
                "top_sort_field": "top_bias",
                "top_market": self.DEFAULT_MARKET,
                "top_fields": {},
            },
            callback_prefixes=[
                "futures_top_sentiment",
                "top_period_",
                "top_sort_",
                "top_limit_",
                "top_sort_field_",
    ...
    async def handle_callback(self, update, context, services: Dict[str, object]) -> bool:
        ...
        data = query.data or ""
        if data in (self.card_id, self.entry_callback, "futures_top_sentiment_refresh"):
            await self._reply(query, h, ensure)
            return True
        if data == "top_nop":
            return True

        if data.startswith("top_sort_field_"):
            h.user_states["top_sort_field"] = data.replace("top_sort_field_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("top_market_"):
            h.user_states["top_market"] = data.replace("top_market_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("top_period_"):
            h.user_states["top_period"] = data.replace("top_period_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("top_sort_"):
            h.user_states["top_sort"] = data.replace("top_sort_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("top_limit_"):
            val = data.replace("top_limit_", "")
            if val.isdigit():
                h.user_states["top_limit"] = int(val)
                await self._edit(query, h, ensure)
                return True
            return False
        if data.startswith("field_top_toggle_"):
            col = data.replace("field_top_toggle_", "")
            fields_state = self._ensure_field_state(h)
            if col in fields_state:
                fields_state[col] = not fields_state[col]
                h.user_states["top_fields"] = fields_state
            await self._edit(query, h, ensure)
            return True
        return False
    ...
    def _build_keyboard(self, h):
        ...
        period = h.user_states.get("top_period", "15m")
        sort_order = h.user_states.get("top_sort", "desc")
        current_limit = h.user_states.get("top_limit", 10)
        current_sort_field = h.user_states.get("top_sort_field", "top_bias")
        market = h.user_states.get("top_market", self.DEFAULT_MARKET)
        ...
        try:
            ...
            for row in metrics:
                ...
                items.append({
                    ...
                    "top_bias": abs(float(row.get("大户偏离") or 0)),
                    "top_momentum": float(row.get("大户情绪动量") or 0),
                    "top_volatility": float(row.get("大户波动") or 0),
    ...
    def _ensure_field_state(self, h) -> Dict[str, bool]:
        state = h.user_states.get("top_fields")
        if not state:
            state = {}
            ...
            for _off in {"quote_volume", "振幅", "成交笔数", "主动买卖比", "price"}:
                if _off in state:
                    state[_off] = False
            h.user_states["top_fields"] = state
        return state
