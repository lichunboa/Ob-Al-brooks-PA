...
class FuturesTakerStreakCard(RankingCard):
    ...
    FALLBACK = "card.taker_streak.fallback"
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            default_state={
                ...
                "tstreak_sort": "desc",
                "tstreak_limit": 10,
                "tstreak_sort_field": "taker_streak",
                "tstreak_market": self.DEFAULT_MARKET,
                "tstreak_fields": {},
            },
            callback_prefixes=[
                "futures_taker_streak",
                "tstreak_period_",
                "tstreak_sort_",
                "tstreak_limit_",
                "tstreak_sort_field_",
                "tstreak_market_",
                "field_tstreak_toggle_",
            ],
            priority=30,
        )
    ...
    async def handle_callback(self, update, context, services: Dict[str, object]) -> bool:
        ...
        data = query.data or ""
        if data in (self.card_id, self.entry_callback, "futures_taker_streak_refresh"):
            await self._reply(query, h, ensure)
            return True
        if data == "tstreak_nop":
            return True

        if data.startswith("tstreak_sort_field_"):
            h.user_states["tstreak_sort_field"] = data.replace("tstreak_sort_field_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("tstreak_market_"):
            h.user_states["tstreak_market"] = data.replace("tstreak_market_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("tstreak_period_"):
            h.user_states["tstreak_period"] = data.replace("tstreak_period_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("tstreak_sort_"):
            h.user_states["tstreak_sort"] = data.replace("tstreak_sort_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("tstreak_limit_"):
            val = data.replace("tstreak_limit_", "")
            if val.isdigit():
                h.user_states["tstreak_limit"] = int(val)
                await self._edit(query, h, ensure)
                return True
            return False
        if data.startswith("field_tstreak_toggle_"):
            col = data.replace("field_tstreak_toggle_", "")
            fields_state = self._ensure_field_state(h)
            if col in fields_state:
                fields_state[col] = not fields_state[col]
                h.user_states["tstreak_fields"] = fields_state
            await self._edit(query, h, ensure)
            return True
        return False
    ...
    async def _build_payload(self, h, ensure, lang=None, query=None):
        if lang is None and query is not None:
            lang = resolve_lang(query)
        period = h.user_states.get("tstreak_period", "15m")
        sort_order = h.user_states.get("tstreak_sort", "desc")
        limit = h.user_states.get("tstreak_limit", 10)
        sort_field = h.user_states.get("tstreak_sort_field", "taker_streak")
        fields_state = self._ensure_field_state(h)
    ...
    def _build_keyboard(self, h):
        ...
        period = h.user_states.get("tstreak_period", "15m")
        sort_order = h.user_states.get("tstreak_sort", "desc")
        current_limit = h.user_states.get("tstreak_limit", 10)
        current_sort_field = h.user_states.get("tstreak_sort_field", "taker_streak")
        market = h.user_states.get("tstreak_market", self.DEFAULT_MARKET)
        ...
        if self.SHOW_MARKET_SWITCH:
            kb.append([
                b("现货", "tstreak_market_spot", active=market == "spot"),
                b("期货", "tstreak_market_futures", active=market == "futures"),
            ])
        ...
        for col_id, label, _ in self.general_display_fields:
            ...
            gen_row.append(InlineKeyboardButton(show_label, callback_data=f"field_tstreak_toggle_{col_id}"))
        kb.append(gen_row)
        ...
        for col_id, label, _ in self.special_display_fields:
            state_on = fields_state.get(col_id, True)
            show_label = label if state_on else f"❎{label}"
            spec_row.append(InlineKeyboardButton(show_label, callback_data=f"field_tstreak_toggle_{col_id}"))
        kb.append(spec_row)

        kb.append([
            b(lbl, f"tstreak_sort_field_{fid}", active=current_sort_field == fid)
        ...
        kb.append([b(p, f"tstreak_period_{p}", active=p == period) for p in periods])

        kb.append([
            b("降序", "tstreak_sort_desc", active=sort_order == "desc"),
            b("升序", "tstreak_sort_asc", active=sort_order == "asc"),
            b("10条", "tstreak_limit_10", active=current_limit == 10),
            b("20条", "tstreak_limit_20", active=current_limit == 20),
            b("30条", "tstreak_limit_30", active=current_limit == 30),
        ])

        kb.append([
            _btn_auto(None, "🏠主菜单", "ranking_menu"),
            _btn_auto(None, "🔄刷新", "futures_taker_streak_refresh"),
        ])
    ...
    def _load_rows(
        ...
        try:
            ...
            for row in metrics:
                ...
                items.append({
                    "symbol": sym,
                    "taker_streak": float(row.get("主动连续根数") or 0),
    ...
    def _ensure_field_state(self, h) -> Dict[str, bool]:
        state = h.user_states.get("tstreak_fields")
        if not state:
            state = {}
            ...
            for _off in {"quote_volume", "振幅", "成交笔数", "主动买卖比", "price"}:
                if _off in state:
                    state[_off] = False
            h.user_states["tstreak_fields"] = state
        return state
