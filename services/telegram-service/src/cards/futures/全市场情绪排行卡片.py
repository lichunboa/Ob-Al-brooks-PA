...
class FuturesCrowdSentimentCard(RankingCard):
    ...
    FALLBACK = "card.market_sentiment.fallback"
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            button_text="🌐 全体情绪",
            button_key="card.crowd_sentiment.btn",
            category="free",
            ...
            default_state={
                ...
                "crowd_sort": "desc",
                "crowd_limit": 10,
                "crowd_sort_field": "crowd_bias",
                "crowd_market": self.DEFAULT_MARKET,
                "crowd_fields": {},
            },
            callback_prefixes=[
                "futures_crowd_sentiment",
                "crowd_period_",
                "crowd_sort_",
                "crowd_limit_",
                "crowd_sort_field_",
        ...
        self.special_display_fields: List[Tuple[str, str, bool]] = [
            ("crowd_ratio", "全体多空比", False),
            ("crowd_bias", "全体偏离", False),
            ("crowd_volatility", "全体波动", False),
        ]
    ...
    async def handle_callback(self, update, context, services: Dict[str, object]) -> bool:
        ...
        data = query.data or ""
        if data in (self.card_id, self.entry_callback, "futures_crowd_sentiment_refresh"):
            await self._reply(query, h, ensure)
            return True
        if data == "crowd_nop":
            return True

        if data.startswith("crowd_sort_field_"):
            h.user_states["crowd_sort_field"] = data.replace("crowd_sort_field_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("crowd_market_"):
            h.user_states["crowd_market"] = data.replace("crowd_market_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("crowd_period_"):
            h.user_states["crowd_period"] = data.replace("crowd_period_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("crowd_sort_"):
            h.user_states["crowd_sort"] = data.replace("crowd_sort_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("crowd_limit_"):
            val = data.replace("crowd_limit_", "")
            if val.isdigit():
                h.user_states["crowd_limit"] = int(val)
                await self._edit(query, h, ensure)
                return True
            return False
        if data.startswith("field_crowd_toggle_"):
            col = data.replace("field_crowd_toggle_", "")
            fields_state = self._ensure_field_state(h)
            if col in fields_state:
                fields_state[col] = not fields_state[col]
                h.user_states["crowd_fields"] = fields_state
            await self._edit(query, h, ensure)
            return True
        return False
    ...
    async def _build_payload(self, h, ensure, lang=None, query=None):
        if lang is None and query is not None:
            lang = resolve_lang(query)
        period = h.user_states.get("crowd_period", "15m")
        sort_order = h.user_states.get("crowd_sort", "desc")
        limit = h.user_states.get("crowd_limit", 10)
        sort_field = h.user_states.get("crowd_sort_field", "crowd_bias")
        fields_state = self._ensure_field_state(h)
        ...
        text = (
            f'{_t("card.crowd_sentiment.title", lang=lang)}\n'
            ...
            f"{aligned}\n"
            "```\n"
            f'{_t("card.crowd_sentiment.hint", lang=lang)}\n'
    ...
    def _build_keyboard(self, h):
        ...
        period = h.user_states.get("crowd_period", "15m")
        sort_order = h.user_states.get("crowd_sort", "desc")
        current_limit = h.user_states.get("crowd_limit", 10)
        current_sort_field = h.user_states.get("crowd_sort_field", "crowd_bias")
        market = h.user_states.get("crowd_market", self.DEFAULT_MARKET)
        ...
        for col_id, label, _ in self.general_display_fields:
            ...
            gen_row.append(InlineKeyboardButton(show_label, callback_data=f"field_crowd_toggle_{col_id}"))
        kb.append(gen_row)
        ...
        for col_id, label in spec_fields:
            state_on = fields_state.get(col_id, True)
            show_label = label if state_on else f"❎{label}"
            spec_row.append(_btn_auto(None, show_label, f"field_crowd_toggle_{col_id}"))
        kb.append(spec_row)

        kb.append([
            b(lbl, f"crowd_sort_field_{fid}", active=current_sort_field == fid)
    ...
    def _load_rows(
        ...
        try:
            ...
            for row in metrics:
                ...
                items.append({
                    "symbol": sym,
                    "crowd_ratio": float(row.get("全体多空比") or 0),
                    "crowd_bias": abs(float(row.get("全体偏离") or 0)),
                    "crowd_volatility": float(row.get("全体波动") or 0),
    ...
    def _ensure_field_state(self, h) -> Dict[str, bool]:
        state = h.user_states.get("crowd_fields")
        if not state:
            state = {}
            ...
            for _off in {"quote_volume", "振幅", "成交笔数", "主动买卖比", "price"}:
                if _off in state:
                    state[_off] = False
            h.user_states["crowd_fields"] = state
        return state
...
CARD = FuturesCrowdSentimentCard()
