...
class FuturesOIZAlertCard(RankingCard):
    """🚩 OI极值告警榜"""

    FALLBACK = "card.oi_alert.fallback"
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            button_text="🚩 OI极值",
            button_key="card.oi_extreme.btn",
            category="free",
            ...
            default_state={
                ...
                "oiz_sort": "desc",
                "oiz_limit": 10,
                "oiz_sort_field": "oi_z_abs",
                "oiz_market": self.DEFAULT_MARKET,
                "oiz_fields": {},
            },
            callback_prefixes=[
                "futures_oi_z_alert",
    ...
    async def handle_callback(self, update, context, services: Dict[str, object]) -> bool:
        ...
        data = query.data or ""
        if data in (self.card_id, self.entry_callback, "futures_oi_z_alert_refresh"):
            await self._reply(query, h, ensure)
            return True
        if data == "oiz_nop":
            return True

        if data.startswith("oiz_sort_field_"):
            h.user_states["oiz_sort_field"] = data.replace("oiz_sort_field_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("oiz_market_"):
            h.user_states["oiz_market"] = data.replace("oiz_market_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("oiz_period_"):
            h.user_states["oiz_period"] = data.replace("oiz_period_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("oiz_sort_"):
            h.user_states["oiz_sort"] = data.replace("oiz_sort_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("oiz_limit_"):
            val = data.replace("oiz_limit_", "")
            if val.isdigit():
                h.user_states["oiz_limit"] = int(val)
                await self._edit(query, h, ensure)
                return True
            return False
        if data.startswith("field_oiz_toggle_"):
            col = data.replace("field_oiz_toggle_", "")
            fields_state = self._ensure_field_state(h)
            if col in fields_state:
                fields_state[col] = not fields_state[col]
                h.user_states["oiz_fields"] = fields_state
            await self._edit(query, h, ensure)
            return True
        return False
    ...
    async def _build_payload(self, h, ensure, lang=None, query=None):
        if lang is None and query is not None:
            lang = resolve_lang(query)
        period = h.user_states.get("oiz_period", "15m")
        sort_order = h.user_states.get("oiz_sort", "desc")
        limit = h.user_states.get("oiz_limit", 10)
        sort_field = h.user_states.get("oiz_sort_field", "oi_z_abs")
        fields_state = self._ensure_field_state(h)
        ...
        text = (
            f'{_t("card.oi_extreme.title", lang=lang)}\n'
            ...
            f"{aligned}\n"
            "```\n"
            f'{_t("card.oi_extreme.hint", lang=lang)}\n'
    ...
    def _build_keyboard(self, h):
        ...
        period = h.user_states.get("oiz_period", "15m")
        sort_order = h.user_states.get("oiz_sort", "desc")
        current_limit = h.user_states.get("oiz_limit", 10)
        current_sort_field = h.user_states.get("oiz_sort_field", "oi_z_abs")
        market = h.user_states.get("oiz_market", self.DEFAULT_MARKET)
        ...
        if self.SHOW_MARKET_SWITCH:
            kb.append([
                b("现货", "oiz_market_spot", active=market == "spot"),
                b("期货", "oiz_market_futures", active=market == "futures"),
            ])
        ...
        for col_id, label, _ in self.general_display_fields:
            ...
            gen_row.append(InlineKeyboardButton(show_label, callback_data=f"field_oiz_toggle_{col_id}"))
        kb.append(gen_row)
        ...
        for col_id, label, _ in self.special_display_fields:
            state_on = fields_state.get(col_id, True)
            show_label = label if state_on else f"❎{label}"
            spec_row.append(InlineKeyboardButton(show_label, callback_data=f"field_oiz_toggle_{col_id}"))
        kb.append(spec_row)

        kb.append([
            b(lbl, f"oiz_sort_field_{fid}", active=current_sort_field == fid)
        ...
        kb.append([b(p, f"oiz_period_{p}", active=p == period) for p in periods])

        kb.append([
            b("降序", "oiz_sort_desc", active=sort_order == "desc"),
            b("升序", "oiz_sort_asc", active=sort_order == "asc"),
            b("10条", "oiz_limit_10", active=current_limit == 10),
            b("20条", "oiz_limit_20", active=current_limit == 20),
            b("30条", "oiz_limit_30", active=current_limit == 30),
        ])

        kb.append([
            _btn_auto(None, "🏠主菜单", "ranking_menu"),
            _btn_auto(None, "🔄刷新", "futures_oi_z_alert_refresh"),
        ])
    ...
    def _ensure_field_state(self, h) -> Dict[str, bool]:
        state = h.user_states.get("oiz_fields")
        if not state:
            state = {}
            ...
            for _off in {"quote_volume", "振幅", "成交笔数", "主动买卖比", "price"}:
                if _off in state:
                    state[_off] = False
            h.user_states["oiz_fields"] = state
        return state
