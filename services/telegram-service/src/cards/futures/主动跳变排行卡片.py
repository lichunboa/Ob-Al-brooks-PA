...
class FuturesTakerJumpCard(RankingCard):
    """⚡ 主动跳变榜"""

    FALLBACK = "card.taker_jump.fallback"
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            button_text="⚡ 主动跳变",
            button_key="card.taker_jump.btn",
            category="free",
            ...
            default_state={
                ...
                "tjump_sort": "desc",
                "tjump_limit": 10,
                "tjump_sort_field": "taker_jump",
                "tjump_market": self.DEFAULT_MARKET,
                "tjump_fields": {},
            },
            callback_prefixes=[
                "futures_taker_jump",
    ...
    async def handle_callback(self, update, context, services: Dict[str, object]) -> bool:
        ...
        data = query.data or ""
        if data in (self.card_id, self.entry_callback, "futures_taker_jump_refresh"):
            await self._reply(query, h, ensure)
            return True
        if data == "tjump_nop":
            return True

        if data.startswith("tjump_sort_field_"):
            h.user_states["tjump_sort_field"] = data.replace("tjump_sort_field_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("tjump_market_"):
            h.user_states["tjump_market"] = data.replace("tjump_market_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("tjump_period_"):
            h.user_states["tjump_period"] = data.replace("tjump_period_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("tjump_sort_"):
            h.user_states["tjump_sort"] = data.replace("tjump_sort_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("tjump_limit_"):
            val = data.replace("tjump_limit_", "")
            if val.isdigit():
                h.user_states["tjump_limit"] = int(val)
                await self._edit(query, h, ensure)
                return True
            return False
        if data.startswith("field_tjump_toggle_"):
            col = data.replace("field_tjump_toggle_", "")
            fields_state = self._ensure_field_state(h)
            if col in fields_state:
                fields_state[col] = not fields_state[col]
                h.user_states["tjump_fields"] = fields_state
            await self._edit(query, h, ensure)
            return True
        return False
    ...
    async def _build_payload(self, h, ensure, lang=None, query=None):
        if lang is None and query is not None:
            lang = resolve_lang(query)
        period = h.user_states.get("tjump_period", "15m")
        sort_order = h.user_states.get("tjump_sort", "desc")
        limit = h.user_states.get("tjump_limit", 10)
        sort_field = h.user_states.get("tjump_sort_field", "taker_jump")
        fields_state = self._ensure_field_state(h)
        ...
        text = (
            f'{_t("card.taker_jump.title", lang=lang)}\n'
            ...
            f"{aligned}\n"
            "```\n"
            f'{_t("card.taker_jump.hint", lang=lang)}\n'
    ...
    def _build_keyboard(self, h):
        ...
        period = h.user_states.get("tjump_period", "15m")
        sort_order = h.user_states.get("tjump_sort", "desc")
        current_limit = h.user_states.get("tjump_limit", 10)
        current_sort_field = h.user_states.get("tjump_sort_field", "taker_jump")
        market = h.user_states.get("tjump_market", self.DEFAULT_MARKET)
        ...
        if self.SHOW_MARKET_SWITCH:
            kb.append([
                b("现货", "tjump_market_spot", active=market == "spot"),
                b("期货", "tjump_market_futures", active=market == "futures"),
            ])
        ...
        for col_id, label, _ in self.general_display_fields:
            ...
            gen_row.append(InlineKeyboardButton(show_label, callback_data=f"field_tjump_toggle_{col_id}"))
        kb.append(gen_row)
        ...
        for col_id, label, _ in self.special_display_fields:
            state_on = fields_state.get(col_id, True)
            show_label = label if state_on else f"❎{label}"
            spec_row.append(InlineKeyboardButton(show_label, callback_data=f"field_tjump_toggle_{col_id}"))
        kb.append(spec_row)

        kb.append([
            b(lbl, f"tjump_sort_field_{fid}", active=current_sort_field == fid)
        ...
        kb.append([b(p, f"tjump_period_{p}", active=p == period) for p in periods])

        kb.append([
            b("降序", "tjump_sort_desc", active=sort_order == "desc"),
            b("升序", "tjump_sort_asc", active=sort_order == "asc"),
            b("10条", "tjump_limit_10", active=current_limit == 10),
            b("20条", "tjump_limit_20", active=current_limit == 20),
            b("30条", "tjump_limit_30", active=current_limit == 30),
        ])

        kb.append([
            _btn_auto(None, "🏠主菜单", "ranking_menu"),
            _btn_auto(None, "🔄刷新", "futures_taker_jump_refresh"),
        ])
    ...
    def _load_rows(
        ...
        try:
            ...
            for row in metrics:
                ...
                items.append({
                    "symbol": sym,
                    "taker_jump": float(row.get("主动跳变幅度") or 0),
    ...
    def _ensure_field_state(self, h) -> Dict[str, bool]:
        state = h.user_states.get("tjump_fields")
        if not state:
            state = {}
            ...
            for _off in {"quote_volume", "振幅", "成交笔数", "主动买卖比", "price"}:
                if _off in state:
                    state[_off] = False
            h.user_states["tjump_fields"] = state
        return state
