...
class FuturesFlipRadarCard(RankingCard):
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            button_text="🛰️ 翻转雷达",
            button_key="card.reversal_radar.btn",
            category="free",
            ...
            default_state={
                ...
                "flip_sort": "desc",
                "flip_limit": 10,
                "flip_sort_field": "flip_signal",
                "flip_market": self.DEFAULT_MARKET,
                "flip_fields": {},
            },
            callback_prefixes=[
                "futures_flip_radar",
        ...
        self.special_display_fields: List[Tuple[str, str, bool]] = [
            ("flip_signal", "翻转信号", False),
            ("top_ratio", "大户多空比", False),
        ]
    ...
    async def handle_callback(self, update, context, services: Dict[str, object]) -> bool:
        ...
        data = query.data or ""
        if data in (self.card_id, self.entry_callback, "futures_flip_radar_refresh"):
            await self._reply(query, h, ensure)
            return True
        if data == "flip_nop":
            return True

        if data.startswith("flip_sort_field_"):
            h.user_states["flip_sort_field"] = data.replace("flip_sort_field_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("flip_market_"):
            h.user_states["flip_market"] = data.replace("flip_market_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("flip_period_"):
            h.user_states["flip_period"] = data.replace("flip_period_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("flip_sort_"):
            h.user_states["flip_sort"] = data.replace("flip_sort_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("flip_limit_"):
            val = data.replace("flip_limit_", "")
            if val.isdigit():
                h.user_states["flip_limit"] = int(val)
                await self._edit(query, h, ensure)
                return True
            return False
        if data.startswith("field_flip_toggle_"):
            col = data.replace("field_flip_toggle_", "")
            fields_state = self._ensure_field_state(h)
            if col in fields_state:
                fields_state[col] = not fields_state[col]
                h.user_states["flip_fields"] = fields_state
            await self._edit(query, h, ensure)
            return True
        return False
    ...
    async def _build_payload(self, h, ensure, lang=None, query=None):
        if lang is None and query is not None:
            lang = resolve_lang(query)
        period = h.user_states.get("flip_period", "15m")
        sort_order = h.user_states.get("flip_sort", "desc")
        limit = h.user_states.get("flip_limit", 10)
        sort_field = h.user_states.get("flip_sort_field", "flip_signal")
        fields_state = self._ensure_field_state(h)
        ...
        text = (
            f'{_t("card.reversal_radar.title", lang=lang)}\n'
            ...
            f"{aligned}\n"
            "```\n"
            f'{_t("card.reversal_radar.hint", lang=lang)}\n'
    ...
    def _build_keyboard(self, h):
        ...
        period = h.user_states.get("flip_period", "15m")
        sort_order = h.user_states.get("flip_sort", "desc")
        current_limit = h.user_states.get("flip_limit", 10)
        current_sort_field = h.user_states.get("flip_sort_field", "flip_signal")
        market = h.user_states.get("flip_market", self.DEFAULT_MARKET)
        ...
        if self.SHOW_MARKET_SWITCH:
            kb.append([
                b("现货", "flip_market_spot", active=market == "spot"),
                b("期货", "flip_market_futures", active=market == "futures"),
            ])
        ...
        for col_id, label, _ in self.general_display_fields:
            ...
            gen_row.append(InlineKeyboardButton(show_label, callback_data=f"field_flip_toggle_{col_id}"))
        kb.append(gen_row)
        ...
        for col_id, label, _ in self.special_display_fields:
            state_on = fields_state.get(col_id, True)
            show_label = label if state_on else f"❎{label}"
            spec_row.append(InlineKeyboardButton(show_label, callback_data=f"field_flip_toggle_{col_id}"))
        kb.append(spec_row)

        kb.append([
            b(lbl, f"flip_sort_field_{fid}", active=current_sort_field == fid)
        ...
        kb.append([b(p, f"flip_period_{p}", active=p == period) for p in periods])

        kb.append([
            b("降序", "flip_sort_desc", active=sort_order == "desc"),
            b("升序", "flip_sort_asc", active=sort_order == "asc"),
            b("10条", "flip_limit_10", active=current_limit == 10),
            b("20条", "flip_limit_20", active=current_limit == 20),
            b("30条", "flip_limit_30", active=current_limit == 30),
        ])

        kb.append([
            _btn_auto(None, "🏠主菜单", "ranking_menu"),
            _btn_auto(None, "🔄刷新", "futures_flip_radar_refresh"),
        ])
    ...
    def _load_rows(
        ...
        try:
            ...
            for row in metrics:
                ...
                items.append({
                    "symbol": sym,
                    "flip_signal": float(row.get("情绪翻转信号") or 0),
                    "top_ratio": float(row.get("大户多空比") or 0),
    ...
    def _ensure_field_state(self, h) -> Dict[str, bool]:
        state = h.user_states.get("flip_fields")
        if not state:
            state = {}
            ...
            for _off in {"quote_volume", "振幅", "成交笔数", "主动买卖比", "price"}:
                if _off in state:
                    state[_off] = False
            h.user_states["flip_fields"] = state
        return state
