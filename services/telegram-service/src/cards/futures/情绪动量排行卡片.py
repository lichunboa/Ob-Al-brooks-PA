...
class FuturesSentimentMomentumCard(RankingCard):
    """🚀 情绪动量榜"""

    FALLBACK = "card.sentiment_mom.fallback"
    ...
    def __init__(self) -> None:
        super().__init__(
            ...
            button_text="🚀 情绪动量",
            button_key="card.sentiment_momentum.btn",
            category="free",
            ...
            default_state={
                ...
                "sm_sort": "desc",
                "sm_limit": 10,
                "sm_sort_field": "top_momentum",
                "sm_market": self.DEFAULT_MARKET,
                "sm_fields": {},
            },
            callback_prefixes=[
                "futures_sentiment_momentum",
                "sm_period_",
                "sm_sort_",
                "sm_limit_",
                "sm_sort_field_",
    ...
    async def handle_callback(self, update, context, services: Dict[str, object]) -> bool:
        ...
        data = query.data or ""
        if data in (self.card_id, self.entry_callback, "futures_sentiment_momentum_refresh"):
            await self._reply(query, h, ensure)
            return True
        if data == "sm_nop":
            return True

        if data.startswith("sm_sort_field_"):
            h.user_states["sm_sort_field"] = data.replace("sm_sort_field_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("sm_market_"):
            h.user_states["sm_market"] = data.replace("sm_market_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("sm_period_"):
            h.user_states["sm_period"] = data.replace("sm_period_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("sm_sort_"):
            h.user_states["sm_sort"] = data.replace("sm_sort_", "")
            await self._edit(query, h, ensure)
            return True
        if data.startswith("sm_limit_"):
            val = data.replace("sm_limit_", "")
            if val.isdigit():
                h.user_states["sm_limit"] = int(val)
                await self._edit(query, h, ensure)
                return True
            return False
        if data.startswith("field_sm_toggle_"):
            col = data.replace("field_sm_toggle_", "")
            fields_state = self._ensure_field_state(h)
            if col in fields_state:
                fields_state[col] = not fields_state[col]
                h.user_states["sm_fields"] = fields_state
            await self._edit(query, h, ensure)
            return True
        return False
    ...
    async def _build_payload(self, h, ensure, lang=None, query=None):
        if lang is None and query is not None:
            lang = resolve_lang(query)
        period = h.user_states.get("sm_period", "15m")
        sort_order = h.user_states.get("sm_sort", "desc")
        limit = h.user_states.get("sm_limit", 10)
        sort_field = h.user_states.get("sm_sort_field", "top_momentum")
        fields_state = self._ensure_field_state(h)
        ...
        text = (
            f'{_t("card.sentiment_momentum.title", lang=lang)}\n'
            ...
            f"{aligned}\n"
            "```\n"
            f'{_t("card.sentiment_momentum.hint", lang=lang)}\n'
    ...
    def _build_keyboard(self, h):
        ...
        period = h.user_states.get("sm_period", "15m")
        sort_order = h.user_states.get("sm_sort", "desc")
        current_limit = h.user_states.get("sm_limit", 10)
        current_sort_field = h.user_states.get("sm_sort_field", "top_momentum")
        market = h.user_states.get("sm_market", self.DEFAULT_MARKET)
        ...
        if self.SHOW_MARKET_SWITCH:
            kb.append([
                b("现货", "sm_market_spot", active=market == "spot"),
                b("期货", "sm_market_futures", active=market == "futures"),
            ])
        ...
        for col_id, label, _ in self.general_display_fields:
            ...
            gen_row.append(InlineKeyboardButton(show_label, callback_data=f"field_sm_toggle_{col_id}"))
        kb.append(gen_row)
        ...
        for col_id, label, _ in self.special_display_fields:
            state_on = fields_state.get(col_id, True)
            show_label = label if state_on else f"❎{label}"
            spec_row.append(InlineKeyboardButton(show_label, callback_data=f"field_sm_toggle_{col_id}"))
        kb.append(spec_row)

        kb.append([
            b(lbl, f"sm_sort_field_{fid}", active=current_sort_field == fid)
        ...
        kb.append([b(p, f"sm_period_{p}", active=p == period) for p in periods])

        kb.append([
            b("降序", "sm_sort_desc", active=sort_order == "desc"),
            b("升序", "sm_sort_asc", active=sort_order == "asc"),
            b("10条", "sm_limit_10", active=current_limit == 10),
            b("20条", "sm_limit_20", active=current_limit == 20),
            b("30条", "sm_limit_30", active=current_limit == 30),
        ])

        kb.append([
            _btn_auto(None, "🏠主菜单", "ranking_menu"),
            _btn_auto(None, "🔄刷新", "futures_sentiment_momentum_refresh"),
        ])
    ...
    def _load_rows(
        ...
        try:
            ...
            for row in metrics:
                ...
                items.append({
                    "symbol": sym,
                    "top_momentum": float(row.get("大户情绪动量") or 0),
    ...
    def _ensure_field_state(self, h) -> Dict[str, bool]:
        state = h.user_states.get("sm_fields")
        if not state:
            state = {}
...
CARD = FuturesSentimentMomentumCard()
