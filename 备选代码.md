...
// 1. 闪卡精确统计
const files = dv.pages("#flashcards");
let s = { single: 0, multi: 0, cloze: 0 };

for (let p of files) {
    const content = await app.vault.read(app.vault.getAbstractFileByPath(p.file.path));
    // 填空: ==...==
    s.cloze += (content.match(/==[^=]+==/g) || []).length;
    // 单行: :: (排除 ::: 这种可能是其他的)
    s.single += (content.match(/^.+?::.+$/gm) || []).length;
    // 多行: 行首的 ? 或 ??
    s.multi += (content.match(/^(?:\>)?\s*\?{1,2}\s*$/gm) || []).length;
}
let total = s.single + s.multi + s.cloze;

// 2. 课程展示 (简化版，无需外部JSON，直接读取笔记元数据)
const courseNotes = dv.pages("#PA/Course").sort(p => p.module_id);
let studied = courseNotes.where(p => p.studied).length;
let progress = courseNotes.length > 0 ? Math.round((studied / courseNotes.length)*100) : 0;

const root = dv.el("div", "", { attr: { style: cfg.card } });
root.innerHTML = `
<div style="display:flex; gap:20px;">
    <div style="flex:1;">
        <div style="font-weight:700; color:${cfg.purple}; margin-bottom:10px;">🧠 记忆库 (Flashcards)</div>
        <div style="display:flex; align-items:baseline; gap:5px; margin-bottom:5px;">
            <span style="font-size:1.8em; font-weight:800;">${total}</span>
            <span style="font-size:0.8em; opacity:0.6;">Cards</span>
        </div>
        <div style="font-size:0.75em; opacity:0.8; display:flex; gap:10px;">
            <span style="color:${cfg.blue}">● 单行 ${s.single}</span>
            <span style="color:${cfg.green}">● 多行 ${s.multi}</span>
            <span style="color:${cfg.purple}">● 填空 ${s.cloze}</span>
        </div>
        <button onclick="app.commands.executeCommandById('obsidian-spaced-repetition:review-flashcards')" style="margin-top:15px; width:100%; background:${cfg.purple}; color:white; border:none; padding:6px; border-radius:6px; cursor:pointer;">⚡️ 开始复习</button>
    </div>

    <div style="flex:1; border-left:1px solid rgba(255,255,255,0.1); padding-left:20px;">
        <div style="font-weight:700; color:#fff; margin-bottom:10px;">📚 课程进度 (Syllabus)</div>
        <div style="font-size:0.8em; margin-bottom:5px; display:flex; justify-content:space-between;">
            <span>完成度</span>
            <span>${studied} / ${courseNotes.length}</span>
        </div>
        <div style="height:8px; background:rgba(255,255,255,0.1); border-radius:4px; overflow:hidden;">
            <div style="width:${progress}%; background:${cfg.blue}; height:100%;"></div>
        </div>
        <div style="margin-top:15px; font-size:0.75em; opacity:0.6;">
            最近学习: ${courseNotes.length > 0 ? courseNotes.last().file.link : "暂无"}
        </div>
    </div>
</div>
`;
...
<div style="display:flex; justify-content:space-between; align-items:center;">
    <div>
        <div style="font-weight:700; color:${cfg.purple}; font-size:1.1em;">🧠 记忆库 (Flashcards)</div>
        <div style="font-size:0.8em; opacity:0.6;">${files.length} Notes Scanned</div>
    </div>
    <div style="text-align:right;">
        <div style="font-size:1.8em; font-weight:800; line-height:1;">${total}</div>
        <div style="font-size:0.7em; opacity:0.5;">Cards</div>
    </div>
</div>

<div style="margin:15px 0; font-size:0.8em; display:flex; gap:15px; opacity:0.8;">
    <span style="color:${cfg.blue}">● 单行: ${s.single}</span>
    <span style="color:${cfg.green}">● 多行: ${s.multi}</span>
    <span style="color:${cfg.purple}">● 填空: ${s.cloze}</span>
</div>

<button onclick="app.commands.executeCommandById('obsidian-spaced-repetition:review-flashcards')" style="width:100%; background:${cfg.purple}; color:white; border:none; padding:8px; border-radius:6px; cursor:pointer; font-weight:bold;">⚡️ 开始复习</button>
`;
```
...
root.innerHTML = `
<div style="font-weight:700; opacity:0.7; margin-bottom:12px;">🚀 快速交易 (New Trade)</div>
<div style="display:flex; align-items:center;">
    ${btn("#10B981", "+ 实盘 (Live)", "quickadd:choice:New Live Trade")}
    ${btn("#3B82F6", "+ 模拟 (Demo)", "quickadd:choice:New Demo Trade")}
    ${btn("#F59E0B", "+ 回测 (Back)", "quickadd:choice:New Backtest")}
</div>
<div style="margin-top:10px; font-size:0.8em; opacity:0.5;">
    * 按钮已关联 QuickAdd，请在插件设置中将 Action 指向 '单笔交易模版'
</div>
`;
```
...
for(const id in app.commands.commands) {
    if(id.includes("repetition") && id.includes("review")) {
        targetCmd = id;
        break;
    }
}
...
let stats = { totalCards: 0, dueToday: 0, future: 0, newCards: 0 };
...
for (let p of files) {
    ...
    // 卡片计数
    const cloze = (content.match(reCloze) || []).length;
    const singleRev = (content.match(reSingleRev) || []).length;
    const allColons = (content.match(reSingleNorm) || []).length;
    const singleNorm = Math.max(0, allColons - singleRev);
    const multiRev = (content.match(reMultiRev) || []).length;
    const multiNorm = (content.match(reMultiNorm) || []).length;

    let fileCards = cloze + singleNorm + (singleRev * 2) + multiNorm + (multiRev * 2);
    stats.totalCards += fileCards;
    
    typeStats.cloze += cloze;
    typeStats.single += (singleNorm + singleRev);
    typeStats.multi += (multiNorm + multiRev);

    // SR 状态解析
    const srTags = content.match(reSR) || [];
    let fileDue = 0; 
    let fileFuture = 0;
    
    for (let tag of srTags) {
        let datePart = tag.substring(8, 18);
        if (datePart && datePart.match(/^\d{4}-\d{2}-\d{2}$/)) {
            if (datePart <= todayStr) fileDue++;
            else fileFuture++;
        }
    }
    stats.dueToday += fileDue;
    stats.future += fileFuture;
}

stats.newCards = Math.max(0, stats.totalCards - stats.dueToday - stats.future);

// 计算进度条
let totalPhys = typeStats.cloze + typeStats.single + typeStats.multi;
let pSingle = totalPhys > 0 ? (typeStats.single/totalPhys*100) : 0;
let pMulti = totalPhys > 0 ? (typeStats.multi/totalPhys*100) : 0;
let pCloze = totalPhys > 0 ? (typeStats.cloze/totalPhys*100) : 0;

// 4. 渲染 (使用数组拼接，避免模板字符串错误)
const root = dv.el("div", "", { attr: { style: cfg.card } });

// 将 HTML 拆分为数组，防止意外的符号解析错误
const htmlParts = [
    '<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:10px;">',
        '<div>',
            '<div style="font-weight:800; color:' + cfg.purple + '; font-size:1.1em;">🧠 记忆库 (Flashcards)</div>',
            '<div style="font-size:0.8em; opacity:0.6; margin-top:4px;">' + files.length + ' Notes Scanned</div>',
        '</div>',
        '<div style="text-align:right;">',
            '<div style="font-size:2em; font-weight:800; line-height:1; color:#fff;">' + stats.totalCards + '</div>',
            '<div style="font-size:0.7em; opacity:0.5;">Total Cards</div>',
        '</div>',
    '</div>',

    '<div style="display:flex; gap:4px; height:6px; width:100%; border-radius:3px; overflow:hidden; margin-bottom:8px; opacity:0.8;">',
        '<div style="width:' + pSingle + '%; background:' + cfg.blue + ';" title="单行"></div>',
        '<div style="width:' + pMulti + '%; background:' + cfg.green + ';" title="多行"></div>',
        '<div style="width:' + pCloze + '%; background:' + cfg.purple + ';" title="填空"></div>',
    '</div>',

    '<div style="display:flex; gap:15px; font-size:0.75em; color:' + cfg.textSub + '; margin-bottom:15px;">',
    ...
    '<div style="display:flex; gap:10px; margin-bottom:15px; background:rgba(0,0,0,0.2); padding:8px; border-radius:8px;">',
        '<div style="flex:1; text-align:center;">',
            '<div style="color:' + cfg.red + '; font-weight:bold;">' + stats.dueToday + '</div>',
            '<div style="font-size:0.6em; opacity:0.6;">Due</div>',
        '</div>',
        '<div style="flex:1; text-align:center; border-left:1px solid rgba(255,255,255,0.1); border-right:1px solid rgba(255,255,255,0.1);">',
            '<div style="color:' + cfg.blue + '; font-weight:bold;">' + stats.newCards + '</div>',
            '<div style="font-size:0.6em; opacity:0.6;">New</div>',
        '</div>',
        '<div style="flex:1; text-align:center;">',
            '<div style="color:' + cfg.green + '; font-weight:bold;">' + stats.future + '</div>',
            '<div style="font-size:0.6em; opacity:0.6;">Future</div>',
        '</div>',
    '</div>',

    '<button id="sr-btn-action" style="width:100%; background:' + cfg.purple + '; color:white; border:none; padding:10px; border-radius:6px; cursor:pointer; font-weight:bold; display:flex; justify-content:center; align-items:center; gap:8px; font-size:0.9em;">',
        '⚡️ 开始复习',
    '</button>',
    '<div style="text-align:center; margin-top:6px; font-size:0.65em; opacity:0.4;">',
        'Command: ' + targetCmd.split(":")[0],
    '</div>'
];
...
// 5. 事件绑定 (最安全的写法)
setTimeout(() => {
    const btn = root.querySelector("#sr-btn-action");
    if(btn) {
...
let typeStats = { single: 0, multi: 0, cloze: 0 };
let folderStats = {}; 
let loadChart = [0,0,0,0,0,0,0]; // 未来7天
let hardCards = []; 
let easeTotal = 0;
let easeCount = 0;
...
htmlParts.push(
    '<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:12px;">',
        '<div>',
            '<div style="font-weight:800; color:' + cfg.purple + '; font-size:1.1em;">🧠 记忆库 (Memory Bank)</div>',
...
htmlParts.push(
    ...
    '<div style="display:flex; gap:12px; font-size:0.7em; color:' + cfg.textSub + '; margin-bottom:15px;">',
        ...
        '<span style="color:' + cfg.green + '">● 多行 ' + typeStats.multi + '</span>',
        '<span style="color:' + cfg.purple + '">● 填空 ' + typeStats.cloze + '</span>',
        '<span style="margin-left:auto; color:' + masteryColor + '; font-weight:bold;" title="平均 Ease 分值">🔥 掌握度: ' + avgEase + '%</span>',
    '</div>'
);
...
if (topFolders.length > 0) {
    ...
    topFolders.forEach(f => {
        folderHtml += '<div style="font-size:0.7em; background:rgba(255,255,255,0.05); padding:3px 8px; border-radius:4px; color:' + cfg.textSub + ';">📂 ' + f.name + ' <span style="color:#fff; font-weight:bold;">' + f.count + '</span></div>';
    });
    folderHtml += '</div>';
...
if (topHard.length > 0) {
    ...
    hardHtml += '<div style="font-size:0.75em; font-weight:bold; color:' + cfg.red + '; margin-bottom:6px;">☠️ 记忆黑洞 (Ease < 230)</div>';
    topHard.forEach(c => {
        hardHtml += 
...
// 6. 按钮
htmlParts.push(
    '<button id="sr-btn-stable" style="width:100%; background:' + cfg.purple + '; color:white; border:none; padding:10px; border-radius:6px; cursor:pointer; font-weight:bold; display:flex; justify-content:center; align-items:center; gap:8px;">⚡️ 开始复习</button>'
);
...
> >    <span style="color:${cfg.blue}">● 单行: ${stats.single}</span>
> >    <span style="color:${cfg.green}">● 多行: ${stats.multi}</span>
> >    <span style="color:${cfg.purple}">● 填空: ${stats.cloze}</span>
> ></div>
> >
...
> >    <div style="font-weight:bold; margin-bottom:2px;">⚡️ 开始复习</div>
> >    <div style="font-size:0.8em; opacity:0.7;">待复习笔记: <b>${reviewFiles.length}</b></div>
> ></div>
> >`;
> >```
...
const processedTrades = trades.map(t => {
    ...
    return {
        ...
        account: (t["账户类型/account_type"] || "Demo").toString(),
        cycle: (t["市场周期/market_cycle"] || "Unknown").toString()
    };
});

// 挂载到全局 window 对象供后续模块调用
window.paData = {
    trades: processedTrades,
    lastUpdated: moment().format("HH:mm:ss")
};

dv.el("div", `数据引擎已就绪 | 更新时间: ${window.paData.lastUpdated}`, { 
...
const renderRow = (label, data, isHeader = false) => {
    ...
    <tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
        ...
        <td style="padding:10px; opacity:0.5;">${data.count} 次</td>
    </tr>`;
};
...
<div style="background:rgba(35,35,35,0.6); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:16px; margin-bottom:20px;">
    <div style="font-weight:700; opacity:0.7; margin-bottom:12px;">📅 周期表现对比 (Cycle Comparison)</div>
    <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.9em;">
        <thead>
            <tr style="opacity:0.4; font-size:0.8em; border-bottom:1px solid rgba(255,255,255,0.1);">
                <th style="padding:5px 10px;">周期</th><th style="padding:5px 10px;">盈亏</th>
                <th style="padding:5px 10px;">胜率</th><th style="padding:5px 10px;">平均R</th>
                <th style="padding:5px 10px;">数量</th>
            </tr>
        </thead>
        <tbody>
            ${renderRow("本周 (This Week)", tw)}
            ${renderRow("上周 (Last Week)", lw)}
            ${renderRow("本月 (This Month)", tm)}
        </tbody>
    </table>
</div>`;
...
for (let t of trades) {
    let rawErr = t["管理错误/management_error"];
...
let html = `
<div style="background:rgba(35,35,35,0.6); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:16px; margin-bottom:20px;">
    <div style="font-weight:700; opacity:0.7; margin-bottom:12px;">💸 违规成本 (Live Tuition)</div>
    <div style="font-size:1.1em; margin-bottom:15px;">总损失: <span style="color:#EF4444; font-weight:800;">-$${totalTuition}</span></div>
    ${sortedErrors.map(e => `
...
dv.el("div", `
<div style="background:rgba(35,35,35,0.6); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:16px; margin-bottom:20px;">
    <div style="font-weight:700; opacity:0.7; margin-bottom:12px;">🖼️ 最新复盘 (Quick View)</div>
...
// 2. 挂载全局数据，后续模块无需重复扫描全库
window.paData = {
    trades: processedTrades,
    lastUpdated: moment().format("HH:mm:ss")
};

dv.el("div", `⚡️ 引擎已就绪 | 更新时间: ${window.paData.lastUpdated}`, { 
    attr: { style: "font-size:0.7em; opacity:0.3; text-align:right; margin-bottom:10px;" } 
});
```
```dataviewjs
...
// 此处保持你原有的 DataviewJS 逻辑，但将统计部分改为读取已加载的数据
const html = `
<div style="${cfg.card}">
    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
        <div>
            <div style="font-weight:800; color:${cfg.purple}; font-size:1.1em;">🧠 知识与记忆 (Fusion Input)</div>
            <div style="font-size:0.8em; opacity:0.6; margin-top:4px;">系统已同步最新学习进度</div>
        </div>
    </div>
    </div>`;
...
const html = `
<div style="${cfg.card}">
    <div style="font-weight:700; opacity:0.7; margin-bottom:12px; color:${cfg.blue}">📊 综合表现对比 (Performance Matrix)</div>
    <table style="width:100%; border-collapse:collapse; text-align:left; font-size:0.9em;">
        <tr style="opacity:0.4; font-size:0.8em; border-bottom:1px solid rgba(255,255,255,0.1);">
            <th style="padding:8px;">周期</th><th style="padding:8px;">盈亏</th><th style="padding:8px;">胜率</th><th style="padding:8px;">平均R</th><th style="padding:8px;">数量</th>
        </tr>
        <tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
            <td style="padding:10px;">本周 (Weekly)</td>
            ...
            <td style="padding:10px; opacity:0.5;">${tw.count} 次</td>
        </tr>
        <tr>
            <td style="padding:10px;">本月 (Monthly)</td>
            ...
            <td style="padding:10px; opacity:0.5;">${tm.count} 次</td>
        </tr>
    </table>
</div>`;
...
let html = `
<div style="${cfg.card}">
    <div style="font-weight:700; opacity:0.7; margin-bottom:12px;">💸 违规学费统计 (Tuition Fee)</div>
