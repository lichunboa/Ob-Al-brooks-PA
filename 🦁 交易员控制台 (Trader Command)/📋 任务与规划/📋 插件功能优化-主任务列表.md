...
- [ ] **3.1 交易日志画廊 (Journal Gallery)**
    ...
    - [x] **重构盈亏统计逻辑 (Refactor PnL to Money)**
        - > **Critical Fix**: 修复了 R 值与金额混淆的严重逻辑缺陷。
        - > **Change**: 数据层严格区分 `pnl` (金额) 与 `r` (比率)。如果只有 R 无风险额，不再强制填充金额为 R 值。
        - > **UI**: 画廊优先显示金额 (如 `+500`)，次要显示 R 值。
    - [x] **实现多币种显示与切换 (Currency Display & Conversion)**
        - > **Feature**: 支持 USD/CNY 切换，所有 PnL 显示金额符号。
        - > **UI**: Dashboard 顶部增加切换开关，全局生效。
