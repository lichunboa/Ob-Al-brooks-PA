...
let useCache = false;
if (!forceReload && window.paData && window.paData.tradesAsc && window.paData.tradesAsc.length > 0) {
    // 深度检查: 确保关键数据结构都存在
    if (window.paData.tradesAsc[0].ticker !== undefined &&
        window.paData.sr && 
...
let srData = {
    total: 0, due: 0, reviewed: 0, avgEase: 0, score: 0, status: "🌱 初始",
    load: {}, // 这是一个必须初始化的对象，否则 View 会报错
    folders: {}, fileList: [],
    cnt: { cloze: 0, sNorm: 0, sRev: 0, mNorm: 0, mRev: 0 },
    quizPool: [], focusFile: null
};
...
if (useCache) {
    ...
    // --- A. 交易数据处理 ---
    const tradePages = dv.pages(`${cfg.tags.trade}`).where(p => !p.file.path.includes(cfg.paths.templates));
    ...
    for (let t of tradePages) {
        ...
        let errStr = utils.getStr(t, ["执行评价/execution_quality", "execution_quality", "管理错误/management_error", "management_error"]);
        
        // 学费统计逻辑优化：Valid Scratch (黄色) 不计入学费
        if (type === "Live" && pnl < 0) {
            let isBadError = !errStr.includes("Perfect") && !errStr.includes("Valid") && !errStr.includes("None") && !errStr.includes("完美") && !errStr.includes("主动");
            if (isBadError) {
        ...
        if (initRisk > 0) {
            ...
            let exit = utils.getVal(t, ["离场/exit_price", "exit_price", "exit"]) || entry;
        ...
        trades.push({
            ...
            signal: utils.getStr(t, ["信号K/signal_bar_quality", "signal_bar_quality"]),
    ...
    const srPages = dv.pages(`${cfg.tags.flashcards} AND -"${cfg.paths.templates}"`);
    let easeSum = 0;
    ...
    await Promise.all(srPages.map(async (p) => {
        try {
            ...
            srData.cnt.cloze += c_cloze; srData.cnt.sRev += c_sRev; srData.cnt.sNorm += c_sNorm; 
            srData.cnt.mRev += c_mRev; srData.cnt.mNorm += c_mNorm;

            // 抓取题目
            let singleMatches = [...clean.matchAll(/^(.+?)::(.+)$/gm)];
            singleMatches.forEach(m => srData.quizPool.push({ q: m[1].trim(), file: p.file.name, path: p.file.path, type: "Basic" }));
            
            // 文件夹归属
            let folderName = p.file.folder.split("/").pop() || "Root";
            if (fileCards > 0) srData.folders[folderName] = (srData.folders[folderName] || 0) + fileCards;

            let fStat = {
                name: p.file.name, path: p.file.path, folder: folderName,
                count: fileCards, due: 0, easeSum: 0, easeCount: 0, avgEase: 250
            };
            ...
            if (fStat.easeCount > 0) fStat.avgEase = Math.round(fStat.easeSum / fStat.easeCount);
    ...
    // 读取大纲文件
    const syFile = app.vault.getFiles().find(f => f.name === cfg.paths.syllabus);
    if (syFile) {
        try {
            const syText = await app.vault.read(syFile);
            const start = syText.indexOf("["); const end = syText.lastIndexOf("]");
            if (start !== -1 && end !== -1) courseData.syllabus = JSON.parse(syText.substring(start, end + 1));
        } catch (e) {}
    }
}
...
if (courseData.syllabus.length > 0) {
    let nextItem = courseData.syllabus.find(s => !courseData.done.has(s.id.toString()));
    if (nextItem) candidates.push({ type: "New", data: nextItem, weight: 30 });
}
if (srData.quizPool.length > 0) {
    for (let i = 0; i < 5; i++) {
        let randQ = srData.quizPool[Math.floor(Math.random() * srData.quizPool.length)];
...
// 4. UI 渲染 (V2.0 悬浮胶囊状态栏)
const rId = "pa-r-" + Date.now();
const hId = "pa-h-" + Date.now();
let stCol = stats.livePnL > 0 ? cfg.colors.live : (stats.livePnL < 0 ? cfg.colors.loss : cfg.colors.textSub);

// 使用绝对定位的悬浮胶囊，不占空间，且非常高级
dv.el("div", `
<div style="
...
">
    <div style="
        ...
        color: ${cfg.colors.textSub};
    ">
        <div style="display:flex; align-items:center; gap:6px;">
            <span>${useCache ? "⚡️" : "🐢"}</span>
            <span style="font-weight:bold; color:${cfg.colors.text}">Engine V17</span>
        </div>
        <div style="width:1px; height:12px; background:rgba(255,255,255,0.1);"></div>
        <div>Live: <b style="color:${stCol}">$${stats.livePnL.toFixed(2)}</b></div>
        <div>Cards: <b>${srData.total}</b></div>
        <div style="width:1px; height:12px; background:rgba(255,255,255,0.1);"></div>
        <div style="display:flex; gap:5px;">
            <button id="${rId}" title="Shuffle Rec" style="background:transparent; border:none; color:${cfg.colors.text}; cursor:pointer; opacity:0.7; padding:2px;">🎲</button>
            <button id="${hId}" title="Force Rescan" style="background:transparent; border:none; color:${cfg.colors.text}; cursor:pointer; opacity:0.7; padding:2px;">↻</button>
        </div>
    </div>
</div>
`);
...
setTimeout(() => {
    const bR = document.getElementById(rId);
    const bH = document.getElementById(hId);
    if(bR) bR.onclick = () => app.commands.executeCommandById("dataview:force-refresh-views");
    if(bH) bH.onclick = () => { new Notice("Scanning Vault..."); window.paForceReload=true; app.commands.executeCommandById("dataview:force-refresh-views"); };
}, 500);