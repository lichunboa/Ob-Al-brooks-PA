...
## 🛡️ v14.6 LTS (2025-12-31)

### 🧬 Agent Skills 生态系统 + 核心回滚 (System Stability)

**类型**: Rollback (回滚) & Feature (新特性) & Architecture (架构)

#### 核心状态 (Kernel State)

- 🔙 **核心回滚至 monolithic version**: 
  - 确认 `pa-core.js` 的模块化拆分 (v5.2) 导致了严重的数据依赖丢失（表现为 SR 计数归零、课程地图失效、图表丢失）。
  - 果断执行 `git checkout` 回滚至 `0f5090e` (v14.6 原始单文件版本)。
  - **结果**: 所有视图瞬间恢复正常，系统重回巅峰稳定性。

#### 新特性: Agent Skills (大脑外挂)

- ✨ **Official Agent Skills Ecosystem**:
  - 在 `.claude/skills/trading-console/` 部署了完整的官方技能组。
  - **System Maintenance**: 懂得如何安全升级代码，且学会了“非必要不重构”的原则。
  - **Trading Analyst**: 即使没有预计算矩阵，也能直接从原始数据中进行即时分析。
  - **System Evolution**: 引入了 `memory/system_evolution.md`，让 Agent 拥有长效记忆，不再犯同样的错误。

#### 经验教训 (Lessons Learned)

- **Lesson**: 对于高度耦合的遗留系统，"渐进式增强" (Agent Skills) 远比 "激进式重构" (Splitting Core) 更安全、更有效。

---

## ⏸️ v5.2 (2025-12-31) [WITHDRAWN]

### 🧪 核心逻辑模块化拆分实验 (Core Decoupling Attempt)

**类型**: Experimental (实验性) - **已撤回**

- **尝试**: 将 `pa-core.js` 拆分为 `pa-loaders.js`, `pa-analyzers.js`, `pa-cache.js`。
- **失败原因**: 原始单文件代码中隐藏了大量“面条式代码”依赖（如 Inline Regex 计数逻辑），拆分后导致上下文丢失。
- **状态**: 代码保留在 `scripts/core/` 但不再被引用。

---

## 🧩 v4.2.9 (2025-12-26)

### ⚔️ Trading Hub 稳定性修复：Today 看板恢复可用

**类型**: Bug Fix (修复) & Stability (稳定性)

- 🐛 **修复 DataviewJS 执行失败**：修复 Trading Hub 中 Today 看板报错 `SyntaxError: Invalid or unexpected token` 导致页面不可用的问题。
- 🧹 **清理残留模板碎片**：移除 `scripts/pa-view-today.js` 中游离在字符串之外的重复 HTML/CSS 片段（会被 JS 解释器当作非法 token）。
- ✅ **校验通过**：使用 `node --check scripts/pa-view-today.js` 验证脚本可被解析，避免再次出现“看起来没改但 Dataview 直接炸”的情况。

## 🧩 v4.2.8 (2025-12-25)

### 📚 太妃课程笔记内容工程：推进到 L05A + YAML/Tab 安全修复

**类型**: Content Ops (内容工程) & Consistency (一致性) & Safety (安全)

- ✅ **逐课手工整理进展**：完成 L04A / L04B / L05A 的“上方结构化总结 + 下方原始笔记（含图片）完整保留”。
- 🧷 **联动字段补齐**：上述课程页补齐 `tags: [PA/Course]`、`module_id`、`studied`、`来源/source` 等控制台联动字段（不新增字段）。
- 🧨 **YAML Tab 修复**：修复多处 frontmatter 列表缩进 Tab，避免 Obsidian 属性显示为无效 / Dataview 解析不稳定。
- 🧹 **正文 Tab 清理**：对 L04A/L05A 正文中残留 Tab 缩进统一为空格，保证一致排版与可维护性。

**涉及文件**:

- `Categories 分类/太妃价格行为/L04A - 突破的原理.md`
- `Categories 分类/太妃价格行为/L04B - 突破后的顺势推动.md`
- `Categories 分类/太妃价格行为/L05A - 回调的内涵.md`

## 🧩 v4.2.7 (2025-12-25)

### 📚 太妃课程笔记内容工程：回滚批量改造 + 逐课手工按模版整理

**类型**: Content Ops (内容工程) & Consistency (一致性) & Safety (安全)

- ⛔️ **停止批量脚本重制**：确认批量模版化会造成“知识点被摘要替代/图片看起来消失（被折叠）”的体验风险，后续改为逐课手工精炼。
- 🔙 **回滚到批量改动前基线**：将 `Categories 分类/太妃价格行为/L*.md` 恢复为原始笔记形态，确保知识点与图片引用保持可见（不使用 `<details>` 折叠隐藏）。
- 🧾 **按模版整理但不覆盖原文**：采用“上方新增 1-4 结构化总结、下方保留原始逐条笔记与图片”的方式，避免丢信息。
- 🧷 **联动字段优先补齐**：为课程笔记补 `来源/source`（NAS 视频链接），确保控制台与课程检索可稳定关联。
- 🧨 **YAML 有效性修复**：避免 frontmatter 使用 Tab 缩进（否则 Obsidian 会提示无效属性）。
- ✅ **已完成示范（手工）**：L01A 已补齐 `来源/source` + 1-4 结构化内容，并追加 `#flashcards`（多类型卡片、`---` 分组分隔）。
...
- ✅ 教练焦点数据层（仅派生数据，不改 UI）：Core 新增 paData.coach，输出 今日/本周/近 30 天 的汇总与“最需要复盘的维度”（基于 negative expectancy + 样本权重），供后续模块智能推荐使用。
- 🧠 统一推荐中枢（交易 > 课程 > 卡片）：Core 新增 `window.paData.recommendations`，将交易教练焦点（含周持续性 Top3 暴露）、课程 hybridRec、SR 到期/题库卡片合并排序，并提供可点击跳转（控制台模块/课程笔记/卡片）。
