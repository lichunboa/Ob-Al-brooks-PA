...
| `r` | （无直接稳定字段） | 新版交易索引**不会**把 `r` 写进 `TradeRecord.pnl`；但 `r` 仍可存在于 `rawFrontmatter`，并会被“复盘提示（Review Hints）”读取用于提示文案。|
| `setup` | （在 `rawFrontmatter` 中） | 新版用于复盘提示/巡检时从 `rawFrontmatter` 读取；导出快照不保证该字段是稳定 schema。|
| `error` | （在 `rawFrontmatter` 中） | 同上。|
