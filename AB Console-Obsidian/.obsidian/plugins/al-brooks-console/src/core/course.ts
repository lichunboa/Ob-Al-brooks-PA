...
export function buildCourseSnapshot(args: {
  ...
  const isDoneCourse = (id: string) =>
    doneSet.has(id) || doneSet.has(simpleCourseId(id));
  const hasNote = (id: string) =>
    Boolean(linksById[id] || linksById[simpleCourseId(id)]);
  ...
  const upNextWindow = Math.max(
    1,
    Math.min(20, Math.floor(args.courseRecommendationWindow || 1))
  );
