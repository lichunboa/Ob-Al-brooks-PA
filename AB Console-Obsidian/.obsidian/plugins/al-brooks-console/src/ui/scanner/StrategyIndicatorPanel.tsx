...
export interface StrategyIndicatorPanelProps {
    ...
    symbol: string;
    symbolName: string;
    interval?: string; // K线周期，例如 "1m", "5m", "15m"
}
...
export const StrategyIndicatorPanel: React.FC<StrategyIndicatorPanelProps> = ({
            ...
            for (const table of pattern.indicatorTables) {
                try {
                    const res = await fetch(
                        `${apiHost}/api/v1/indicators/table/${encodeURIComponent(table)}?symbol=${symbol}&interval=${interval}&limit=1`
                    );
                    if (!res.ok) continue;
    ...
    React.useEffect(() => {
        ...
        setAlerts([]);
        checkPatterns();
        const timer = setInterval(checkPatterns, 60000); // 1分钟检查一次
        return () => clearInterval(timer);
    }, [checkPatterns, interval]); // interval 变化时重新检测
