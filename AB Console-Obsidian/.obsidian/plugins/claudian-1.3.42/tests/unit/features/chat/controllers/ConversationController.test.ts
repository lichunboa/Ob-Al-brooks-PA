...
describe('ConversationController - Async Subagent Restoration', () => {
  let controller: ConversationController;
  let deps: ConversationControllerDeps;
  let mockStatusPanel: any;
  ...
  beforeEach(() => {
    ...
    deps = createMockDeps({
      getStatusPanel: () => mockStatusPanel,
    });
    controller = new ConversationController(deps);
  });

  describe('loadActive - restoreAsyncSubagentsToPanel', () => {
    it('should restore async subagents when loading conversation', async () => {
      deps.state.currentConversationId = 'conv-1';
      (deps.plugin.getConversationById as jest.Mock).mockResolvedValue({
        id: 'conv-1',
        messages: [{
      ...
      expect(mockStatusPanel.restoreSubagents).toHaveBeenCalledWith(
        expect.arrayContaining([
          expect.objectContaining({ id: 'task-1', mode: 'async' }),
        ])
      );
    });

    it('should call clearSubagents if no async subagents in messages', async () => {
      deps.state.currentConversationId = 'conv-1';
      (deps.plugin.getConversationById as jest.Mock).mockResolvedValue({
        id: 'conv-1',
        messages: [{
      ...
      expect(mockStatusPanel.clearSubagents).toHaveBeenCalled();
    });
  });

  describe('switchTo - restoreAsyncSubagentsToPanel', () => {
    it('should restore async subagents when switching conversations', async () => {
      deps.state.currentConversationId = 'old-conv';
      (deps.plugin.switchConversation as jest.Mock).mockResolvedValue({
        id: 'new-conv',
        messages: [{
      ...
      expect(mockStatusPanel.restoreSubagents).toHaveBeenCalledWith(
        expect.arrayContaining([
          expect.objectContaining({ id: 'task-2', mode: 'async' }),
        ])
      );
    });
  });
...
describe('ConversationController - clearTerminalSubagentsFromMessages', () => {
  let controller: ConversationController;
  let deps: ConversationControllerDeps;
  ...
  it('should remove completed async subagents from messages', () => {
    deps.state.currentConversationId = 'conv-1';
    deps.state.messages = [{
      id: '1',
      role: 'assistant',
      ...
      timestamp: Date.now(),
      subagents: [
        { id: 'running-1', mode: 'async', asyncStatus: 'running', description: '', isExpanded: false, status: 'running' as const, toolCalls: [] },
        { id: 'completed-1', mode: 'async', asyncStatus: 'completed', description: '', isExpanded: false, status: 'completed' as const, toolCalls: [] },
      ],
    }];

    controller.clearTerminalSubagentsFromMessages();

    expect(deps.state.messages[0].subagents).toHaveLength(1);
    expect(deps.state.messages[0].subagents![0].id).toBe('running-1');
    // save() is called fire-and-forget, updateConversation will be called asynchronously
  });

  it('should remove error async subagents from messages', () => {
    deps.state.currentConversationId = 'conv-1';
    deps.state.messages = [{
      id: '1',
      role: 'assistant',
      ...
      timestamp: Date.now(),
      subagents: [
        { id: 'pending-1', mode: 'async', asyncStatus: 'pending', description: '', isExpanded: false, status: 'running' as const, toolCalls: [] },
        { id: 'error-1', mode: 'async', asyncStatus: 'error', description: '', isExpanded: false, status: 'error' as const, toolCalls: [] },
      ],
    }];
    ...
    expect(deps.state.messages[0].subagents).toHaveLength(1);
    expect(deps.state.messages[0].subagents![0].id).toBe('pending-1');
  });

  it('should not modify sync subagents', () => {
    deps.state.currentConversationId = 'conv-1';
    deps.state.messages = [{
      id: '1',
      role: 'assistant',
      ...
      timestamp: Date.now(),
      subagents: [
        { id: 'sync-1', mode: undefined, status: 'completed' as const, description: '', isExpanded: false, toolCalls: [] },
        { id: 'async-1', mode: 'async', asyncStatus: 'completed', description: '', isExpanded: false, status: 'completed' as const, toolCalls: [] },
      ],
    }];
    ...
    expect(deps.state.messages[0].subagents).toHaveLength(1);
    expect(deps.state.messages[0].subagents![0].id).toBe('sync-1');
  });

  it('should not save if no subagents were modified', () => {
    deps.state.currentConversationId = 'conv-1';
    deps.state.messages = [{
      id: '1',
      role: 'assistant',
  ...
  it('should not save if no conversation is active', () => {
    deps.state.currentConversationId = null;
    deps.state.messages = [{
      id: '1',
      role: 'assistant',
  ...
  it('should leave empty subagents array when all removed', () => {
    deps.state.currentConversationId = 'conv-1';
    deps.state.messages = [{
      id: '1',
      role: 'assistant',
    ...
    // The implementation filters the array, leaving an empty array (not undefined)
    expect(deps.state.messages[0].subagents).toEqual([]);
  });
});
