```dataviewjs
/* === 🦁 PA Core Engine v13.2 (Refresh Button) === 
   更新：底部增加手动刷新按钮，方便实时同步数据
   功能：全库清洗、智能评分、正则修复
*/
...
<div style="font-size: 0.75em; opacity: 0.6; text-align: right; padding: 5px 0; border-bottom: 1px solid rgba(255,255,255,0.1); font-family: monospace; margin-bottom: 10px; display:flex; justify-content:flex-end; align-items:center;">
    <span style="margin-right: 15px;">🦁 v13.2</span>
    ...
    <button id="${refreshBtnId}" style="
        ...
        cursor:pointer; padding:2px 8px; border-radius:4px; font-size:0.9em;
        transition: all 0.2s;
    ">↻ 刷新</button>
</div>
`);

// 绑定点击事件：强制刷新 Dataview
setTimeout(() => {
    const btn = document.getElementById(refreshBtnId);
    if (btn) {
        btn.onclick = () => {
            new Notice("正在重新抓取数据...");
            // 调用 Obsidian Dataview 的原生刷新命令
