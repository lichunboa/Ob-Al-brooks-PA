...
要尊重market cycle，特别是strong trend的climax，如果要reverse，一般要先演变成TR。第一次看到的PB几乎不可能是反方向的trend，或许是TR中的一个leg，或者final flag。

强势的trend需要market cycle逐渐过渡Trend → TR → Trend；TR之所以是TR是因为有up/down，不是第一次PB就一蹴而就的。
...
- **Open Climax**
    ...
    如果前一天是个某trend主宰的trend day，
    
    - **50%**机率：下一天**open**或者**1h后**可能会有趋势继续
    - **75%**机率：在**开盘2h前，**会有TBTL side way to reversal
    </aside>
    
    ![image.png](42C%20Minor%20climactic%20reversals/image%203.png)
    
- 关闭exhaustion bar往往是final flag
    
    <aside>
    💡
    
    市场周期：TREND →TR→TREND，reversal from climax可能更大概率只是一个minor reversal的final flag
    
    </aside>
    
    ![image.png](42C%20Minor%20climactic%20reversals/image%204.png)
    
- **exhaustion bar→measuring bar [KEEP MM GAP OPEN!]**
    
    这里一个bull trend 持续了蛮久，然后late stage出现了buy climax，而且有很长的上影线，而且位于昨日的TTR区域是个阻力。遇到S&R前出现climax然后变成exhaustion是个很常见的组合，但是不要忽视变成Measuring Gap的可能性，也即创造出一个gap。
    
    因此能否创造出gap，是判断exhaustion或者MM GAP的重要的线索！
