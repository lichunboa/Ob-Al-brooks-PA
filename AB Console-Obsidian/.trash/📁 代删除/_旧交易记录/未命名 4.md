...
```
```dataviewjs
/* === 🦁 PA Data Inspector (Fix Version) === 
   用途：可视化检查 window.paData 中的所有数据
   修复：解决了 "Illegal return statement" 报错
*/

// 1. 核心判断逻辑：如果有数据，则渲染；如果没有，则报错
if (window.paData) {
    ...
    dv.header(1, "🕵️ 数据巡检报告 (Inspector)");
    dv.paragraph(`> **状态**: ✅ 引擎在线 | **更新时间**: ${D.updateTime} | **耗时**: ${D.loadTime}`);

    // ------------------------------------------------
    // 1. 交易数据核对
    // ------------------------------------------------
    dv.header(2, "1. 交易数据核对 (Trades)");

    // A. 账户概览
    dv.table(
        ["账户类型", "交易笔数", "累计盈亏 (PnL)", "胜场数"],
        [
            ["🟢 实盘 (Live)", stats.liveCount, `$${stats.livePnL.toFixed(2)}`, stats.liveWin],
            ["🔵 模拟 (Demo)", D.trades.filter(t => t.type==="Demo").length, "--", "--"],
            ["🟠 回测 (Backtest)", D.trades.filter(t => t.type==="Backtest").length, "--", "--"]
        ]
    );

    // B. 详细交易清单 (前 30 笔)
    dv.header(3, "交易明细 (Top 30)");
    dv.paragraph("请核对 **R 值** 和 **错误标记**：");

    dv.table(
        ["日期", "交易", "类型", "盈亏", "R 值 (智能)", "策略", "错误标记"],
        D.trades.slice(0, 30).map(t => [
            t.date,
            t.link,
            t.type === "Live" ? "🟢 Live" : (t.type === "Backtest" ? "🟠 Back" : "🔵 Demo"),
            t.pnl > 0 ? `<span style='color:#4caf50'>+$${t.pnl}</span>` : (t.pnl < 0 ? `<span style='color:#ff6b6b'>$${t.pnl}</span>` : t.pnl),
            `**${t.r.toFixed(2)} R**`,
            t.setup,
            t.error ? `<span style='color:#ff6b6b'>${t.error}</span>` : `<span style='opacity:0.2'>-</span>`
        ])
    );

    // C. 学费统计
    dv.header(3, "错误的代价 (Tuition Fee)");
    if (stats.tuition > 0) {
        let errRows = Object.entries(stats.errors)
            .sort((a, b) => b[1] - a[1])
            .map(([err, cost]) => [err, `-$${cost}`]);
        
        dv.table(["错误类型", "累计亏损金额"], errRows);
        dv.paragraph(`**总学费**: <span style='color:#ff6b6b; font-weight:bold;'>-$${stats.tuition}</span>`);
    } else {
        dv.paragraph("🎉 暂无因执行错误导致的实盘亏损。");
    }

    // ------------------------------------------------
    // 2. 记忆库数据核对
    // ------------------------------------------------
    dv.header(2, "2. 记忆库数据 (Flashcards)");

    // A. 核心指标
    dv.table(
        ["指标", "数值", "说明"],
        [
            ["物理总数 (Total)", `**${sr.total}**`, "已剔除代码块干扰"],
            ["待复习 (Due)", `<span style='color:${sr.due>0?'#ff6b6b':'inherit'}'>${sr.due}</span>`, "今日到期"],
            ["已复习 (Reviewed)", sr.reviewed, "有复习记录的卡片"],
            ["平均轻松度 (Ease)", `${sr.avgEase}%`, "难度系数"]
        ]
    );

    // B. 文件夹分布
    let folderRows = Object.entries(sr.folders)
        .sort((a, b) => b[1] - a[1])
        .map(([name, count]) => [name, count]);
    if (folderRows.length > 0) dv.table(["文件夹", "卡片数量"], folderRows);

    // C. 未来负载
    dv.header(3, "未来 7 天负载预测");
    let loadRows = Object.entries(sr.load)
        .sort((a, b) => a[0].localeCompare(b[0]))
        .slice(0, 7)
        .map(([date, count]) => [date, `${count} 张`]);

    if (loadRows.length > 0) {
        dv.table(["日期", "预计到期"], loadRows);
    } else {
        dv.paragraph("未来 7 天暂无积压。");
    }

    // ------------------------------------------------
    // 3. 课程进度核对
    // ------------------------------------------------
    dv.header(2, "3. 课程进度 (Syllabus)");
    let doneList = Array.from(D.course.done);
    dv.paragraph(`**已完成模块**: ${doneList.length} 个`);
    
    let mapPreview = Object.entries(D.course.map).slice(0, 5).map(([id, link]) => [id, link]);
    if (mapPreview.length > 0) dv.table(["模块 ID", "关联笔记 (Preview)"], mapPreview);

} else {
    // =============== 如果没有数据，显示错误提示 ===============
    dv.el("div", `
    <div style="background:rgba(255, 107, 107, 0.2); color:#ff6b6b; padding:15px; border-radius:8px; border:1px solid #ff6b6b;">
        <h3>⚠️ 未检测到引擎数据</h3>
        <p>请确保你已经将 <b>Engine V12.0</b> 的代码块放在了本笔记的<b>最上方</b>。</p>
        <p>DataviewJS 代码块是从上往下执行的，Engine 必须先运行。</p>
    </div>
    `);
}