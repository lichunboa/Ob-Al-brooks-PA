```dataviewjs
/* === 🦁 PA Core Engine v13.9 (Recommendation Fix) === 
   更新：修复推荐逻辑，即使无笔记也能推荐大纲内容
   功能：全库清洗、智能评分、交互增强、大纲同步
*/
...
await Promise.all(srPages.map(async (p) => {
    try {
        ...
        if (fileStat.easeCount > 0) fileStat.avgEase = Math.round(fileStat.easeSum / fileStat.easeCount);
...
if (candidateFiles.length > 0) {
    ...
    srData.focusFile = candidateFiles[0];
}
...
let courseData = { done: new Set(), map: {}, syllabus: [], nextRec: null };
...
// B. 读取 JSON 大纲文件
const syFile = app.vault.getAbstractFileByPath("PA_Syllabus_Data.md");
if (syFile) {
    try {
    ...
    } catch(e) { console.log("Syllabus Parse Error", e); }
}

// C. 计算推荐 (Next Recommendation)
// 规则：在 syllabus 中找到第一个不在 done 里的 ID
if (courseData.syllabus.length > 0) {
    ...
    if (nextItem) {
        courseData.nextRec = {
            id: nextItem.id,
            title: nextItem.t,
            link: courseData.map[nextItem.id.toString()] || null // 尝试关联笔记(如果有)
        };
    }
}
...
<div style="font-size: 0.75em; opacity: 0.6; text-align: right; padding: 5px 0; border-bottom: 1px solid rgba(255,255,255,0.1); font-family: monospace; margin-bottom: 10px; display:flex; justify-content:flex-end; align-items:center;">
    <span style="margin-right: 15px;">🦁 v13.9</span>
