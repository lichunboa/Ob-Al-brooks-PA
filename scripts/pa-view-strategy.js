...
if (window.paData) {
  ...
  root.innerHTML = `
    ...
    <svg viewBox="0 0 ${width} ${height}" style="width:100%; height:150px; background:rgba(0,0,0,0.2); border-radius:8px; overflow:visible;">
        ...
        <polyline points="${ptsBack}" fill="none" stroke="${
    c.back
  }" stroke-width="1.5" opacity="0.6" stroke-dasharray="2" />
        <polyline points="${ptsDemo}" fill="none" stroke="${
    c.demo
  }" stroke-width="1.5" opacity="0.8" />
        <polyline points="${ptsLive}" fill="none" stroke="${
    c.live
  }" stroke-width="2.5" />
        
        ${
          curves.live.length > 1
            ? `<circle cx="${ptsLive.split(" ").pop().split(",")[0]}" cy="${
                ptsLive.split(" ").pop().split(",")[1]
              }" r="3" fill="${c.live}" />`
            : ""
        }
    </svg>
                  ...
                  .map(
                    ...
                    <div style="display:flex; justify-content:space-between; font-size:0.85em; background:rgba(255,255,255,0.03); padding:4px 8px; border-radius:4px;">
                        ...
                        <span><span style="color:${
                          s.wr > 50 ? c.live : c.back
                        }">${s.wr}%</span> <span style="opacity:0.4">(${
                      s.total
                    })</span></span>
                    </div>
                `
