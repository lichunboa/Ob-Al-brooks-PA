...
for (let c of syllabus) {
    ...
    if (!doneSet.has(c.id) && !doneSet.has(simpleId)) { next = c; break; }
}
if (!next && syllabus.length > 0) { let randomIndex = Math.floor(Math.random() * syllabus.length); next = syllabus[randomIndex]; recommendationType = "Review"; }
