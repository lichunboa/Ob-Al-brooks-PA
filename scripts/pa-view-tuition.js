...
root.innerHTML = `<div style="font-weight:700; opacity:0.7;">💸 错误的代价 (Tuition)</div><div style="opacity:0.8;">学费总计: <b>${stats.tuition || 0}</b></div>`;
const basePath = app.vault.adapter.basePath;
const cfg = require(basePath + "/Scripts/pa-config.js");
const c = cfg.colors;
...
if (window.paData) {
    ...
    if (stats.tuition === 0) {
        ...
        root.innerHTML += `
        <div style="font-size:0.9em; margin-bottom:10px; opacity:0.8;">因执行错误共计亏损: <span style="color:${c.loss}; font-weight:800; font-size:1.2em;">$${stats.tuition}</span></div>
