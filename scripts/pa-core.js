...
if (useCache) {
    ...
    await Promise.all(srPages.map(async (p) => {
        try {
            ...
            let fStat = { name: p.file.name, path: p.file.path, folder: folderName, count: fileCards, due: 0, easeSum: 0, easeCount: 0, avgEase: 250 };
...
let pnlColor = stats.livePnL > 0 ? cfg.colors.live : (stats.livePnL < 0 ? cfg.colors.loss : "inherit");
...
<div style="font-size: 0.75em; opacity: 0.6; text-align: right; padding: 5px 0; border-bottom: 1px solid rgba(255,255,255,0.1); font-family: monospace; margin-bottom: 10px; display:flex; justify-content:flex-end; align-items:center; gap:10px;">
    ...
    <span>Live: <strong style="color:${pnlColor}">$${stats.livePnL.toFixed(2)}</strong></span>
...
setTimeout(() => {
    ...
    if (btnRefresh) btnRefresh.onclick = () => app.commands.executeCommandById("dataview:force-refresh-views");
