...
if (window.paData) {
  ...
  for (let t of tradesAsc) {
    ...
    if (acct === "live") {
      cum.live += pnl;
      curves.live.push(cum.live);
    } else if (acct === "demo") {
      cum.demo += pnl;
      curves.demo.push(cum.demo);
    } else if (acct === "backtest") {
      cum.back += pnl;
      curves.back.push(cum.back);
    }
  ...
  function getPoints(data) {
    if (data.length < 2)
      return `${padding},${height - padding} ${width},${height - padding}`;
  ...
  const endDate =
    tradesAsc.length > 0 ? tradesAsc[tradesAsc.length - 1].date : "";
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (hasTrade) {
      ...
      // 底部账户类型条
      let bars = "";
      if (data.types.has("Live"))
        bars += `<div style="flex:1; background:${c.live}; border-radius:1px;"></div>`;
      if (data.types.has("Demo"))
        bars += `<div style="flex:1; background:${c.demo}; border-radius:1px;"></div>`;
      if (data.types.has("Backtest"))
        bars += `<div style="flex:1; background:${c.back}; border-radius:1px;"></div>`;
  ...
  const recentTrades = tradesAsc
    .filter((t) => (t.type || "").toLowerCase() === "live")
    .slice(-30);
  let maxR = Math.max(...recentTrades.map((t) => Math.abs(t.r || 0))) || 1;
  let avgR = (
    recentTrades.reduce((acc, t) => acc + (t.r || 0), 0) /
