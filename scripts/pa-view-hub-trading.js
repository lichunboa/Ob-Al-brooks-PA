...
if (window.paData) {
    ...
    // 左侧 (2/3): 今日看板
    // 右侧 (1/3): 快速行动 + 趋势指标
    const root = dv.el("div", "", { attr: { style: "display: grid; grid-template-columns: 2fr 1fr; gap: 20px;" } });
    ...
    <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:15px;">
        ...
        <div style="text-align:right;">
            <div style="font-size:1.8em; font-weight:900; color:${todayPnL >= 0 ? c.live : c.loss};">${todayPnL > 0 ? "+" : ""}${todayPnL}</div>
    ...
    if (activeTrade) {
        leftCol.innerHTML += `
        <div style="flex:1; background:rgba(255,255,255,0.03); border-radius:8px; padding:15px; border:1px solid ${c.accent};">
            <div style="color:${c.accent}; font-weight:bold; margin-bottom:10px;">⚡️ 进行中: ${activeTrade.file.link}</div>
            <div style="font-size:0.9em; opacity:0.8;">
                <div>方向: ${activeTrade["方向/direction"] || "-"}</div>
                <div>形态: ${activeTrade["观察到的形态/patterns_observed"] || "-"}</div>
            </div>
        </div>`;
    } else {
        leftCol.innerHTML += `
    ...
    if (recentTrades.length > 0) {
        ...
        for (let t of recentTrades) {
            ...
            bars += `<div style="flex:1; height:${h}px; background:${color}; border-radius:2px; opacity:${r>=0?1:0.7};" title="R: ${r}"></div>`;
        }
    } else {
