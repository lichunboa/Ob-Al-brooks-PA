...
for (let p of dvPages) {
    ...
    if (cache.frontmatter) {
        ...
        for (let key in cache.frontmatter) {
            ...
            let valStr = (val === undefined || val === null) ? "null" : val.toString().trim();
...
if (window.paData && window.paData.trades) {
    ...
    distData.exec = Object.entries(execMap).sort((a,b) => b[1]-a[1]).slice(0, 5);
...
panelDash.innerHTML = `
    ...
    ${createCard("待修异常", scanStats.issues, scanStats.issues > 0 ? c.loss : c.textSub)}
...
if (scanStats.issues > 0) {
    ...
    issueList.forEach(item => {
        ...
        // 原生跳转事件
        link.addEventListener("click", () => app.workspace.openLinkText(item.path, "", true));
...
Object.entries(tagMap).sort((a,b) => b[1] - a[1]).forEach(([tag, count]) => {
    ...
    span.addEventListener("click", () => {
        app.internalPlugins.plugins['global-search'].instance.openGlobalSearch(`tag:${tag}`);
    });
