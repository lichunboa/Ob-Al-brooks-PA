...
用法:
  # 单品种多周期
  python chart_gen.py -s BTCUSDT -i 5m,15m,1d

  # patrol-l1 批量（5 品种 × 3 周期）
  python chart_gen.py --patrol --port 8094

  # 清理 3 天前的图片
  python chart_gen.py --cleanup --keep-days 3
...
# ab_* 模块路径（避免包导入链）
_INDICATORS_DIR = Path(__file__).resolve().parent.parent / "services/trading-service/src/indicators/batch"
...
CHARTS_ROOT = _PROJECT_ROOT / "data" / "charts"
    ...
    if include_daily:
        ...
        if daily_path.exists():
            ...
            if mtime.date() == datetime.now().date():
                print(f"  ℹ️  1d today already generated")
