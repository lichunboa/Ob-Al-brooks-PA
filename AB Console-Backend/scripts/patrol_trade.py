...
BOT_ID = "claude-pa"
LOG_PATH = "data/pa_trader/pa_trader.log"
...
def validate_equation(eq_str: str) -> tuple:
    ...
    try:
        parts = eq_str.upper().replace(" ", "").split("PXR=")
        if len(parts) != 2:
            parts = eq_str.upper().replace(" ", "").split("P×R=")
        pxr = float(parts[1])
        if pxr < 1.2:
            return False, f"P×R={pxr} < 1.2 门槛，不应开仓"
        return True, f"P×R={pxr} >= 1.2 ✓"
    except (ValueError, IndexError):
        return False, f"无法解析 Trader's Equation: '{eq_str}'，格式应为 'P=55% R=2.5 PxR=1.38'"
...
def validate_stop_loss(side: str, entry: float, sl: float, tp: float,
                       strategy: str = "") -> tuple:
    """校验止损止盈合理性 — R:R 按策略类型分档 (Al Brooks 4-evaluation)
    顺势 (60% prob): R:R >= 1.0 即可盈利
    反转 (40% prob): R:R >= 2.0 才有正期望
    """
    if side == "BUY":
        if sl >= entry:
    ...
    # Al Brooks 40-60 Rule: 概率与 R:R 反比
    reversal_keywords = ["MTR", "REVERSAL", "DT", "DB", "HSB", "WEDGE_REV"]
    is_reversal = any(kw in strategy.upper() for kw in reversal_keywords)
    min_rr = 2.0 if is_reversal else 1.0
    label = "反转(40%prob需≥2:1)" if is_reversal else "顺势(60%prob需≥1:1)"

    if rr < min_rr:
        return False, f"盈亏比{rr:.2f} < {min_rr} [{label}]"
    return True, f"Risk={risk:.2f} Reward={reward:.2f} R:R={rr:.2f} [{label}] ✓"
...
def main():
    ...
    ok, msg = validate_stop_loss(args.side, args.entry, args.sl, args.tp, args.strategy)
