...
用法:
  python3 scripts/patrol_scan.py                         # 全品种扫描摘要
  python3 scripts/patrol_scan.py --detail BNBUSDT        # 某品种详细5m K线
  python3 scripts/patrol_scan.py --detail BNBUSDT --tf 15m  # 指定时间框架
  python3 scripts/patrol_scan.py --positions              # 持仓管理摘要
  python3 scripts/patrol_scan.py --full                   # 摘要 + 持仓 + 候选详情
"""
...
def compute_ai(bars: list, ema20: float, atr14: float) -> tuple:
    ...
    # 归一化到 [-1, +1]
    momentum = weighted_sum / (norm * 55) if norm else 0  # 55 = sum(1..10)
    ...
    # ── 信号3: 窗口内结构 ──
    window_high = max(b["H"] for b in recent)
    window_low = min(b["L"] for b in recent)
