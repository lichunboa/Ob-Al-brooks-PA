...
这轮新增核对了 4 组直接和当前瓶颈对应的 Brooks 页图：

1. `If now channel, trade like channel`
   - 页图：`advanced_trade_like_channel-0290.png`
   - 含义：进入通道后，顺势恢复要按通道处理，不能继续按“必须强 breakout follow-through”处理。
2. `When channel is broad / tight`
   - 页图：`advanced_bull_channel_tight-0409.png`
   - 页图：`advanced_bear_channel_tight-0494.png`
   - 含义：紧密通道和宽通道必须分开，宽通道里的回调与反转不应和 tight channel 一样被压死。
3. `Bear channel 2nd Leg Trap in TR`
   - 页图：`advanced_second_leg_trap-0605.png`
   - 含义：`second-leg trap` 要保留，但不能把所有宽通道里的 `高1/高2/楔形/头肩/双顶双底` 都粗暴压成它。
...
1. `宽通道 / 弱趋势` 里的顺势恢复
   - 现在允许：
     ...
     - `exhaustion_detected`
     - 强 `H1/H2`
   - 这些都可以作为 `follow-through` 之外的合法放行证据。
2. `H1/L1` 与 `H2/L2`
   - 不再把“信号成熟度”和“目标路径是否通畅”绑成同一个 veto。
   - 先判断 setup 是否成熟，再让 `prior_level / measured_move` 去单独决定是否拦截。
3. `看衰突破`
   - 如果已经回到区间内，且有强信号棒/接受，不再强制要求把失败突破证据堆得过满。
4. `宽通道反转 vs second-leg trap`
   - `楔形 / 头肩` 只有在 `stairs / exhaustion / failed breakout / trendline break / trapped side` 这些 Brooks 背景之一成立时，才不再被自动压回 `second-leg trap`。

## 二十、这轮多窗口复测结果

这一轮改的是：

1. `prior_level` 在顺势恢复链里的第一目标选择
2. `H2/L2` 在 `TR / weak trend` 里的 second-entry 放行边界
3. `second-leg trap` 与 `楔形 / 头肩 / 双顶双底` 的分界

复测仍然用分散窗口，不按单一品种或单一周期特调：

1. `BTCUSDT 5m 2022-01-24 ~ 2022-02-23`
   - 上一轮：`567` 笔，`18.9` 笔/天，PF `0.45`
   - 现在：`705` 笔，`23.5` 笔/天，PF `0.45`
2. `BTCUSDT 5m 2025-01-10 ~ 2025-02-09`
   - 上一轮：`520` 笔，`17.33` 笔/天，PF `0.43`
   - 现在：`652` 笔，`21.73` 笔/天，PF `0.35`
3. `BTCUSDT 15m 2022-01-24 ~ 2022-02-23`
   - 上一轮：`96` 笔
   - 现在：`98` 笔，PF `0.86`
4. `ETHUSDT 15m 2023-07-10 ~ 2023-08-09`
   - 上一轮：`113` 笔
   - 现在：`113` 笔，PF `0.49`
5. `SOLUSDT 15m 2025-01-10 ~ 2025-02-09`
   - 上一轮：`91` 笔
   - 现在：`94` 笔，PF `0.70`
6. `BTCUSDT 15m 2025-11-01 ~ 2025-11-30`
   - 上一轮：`104` 笔
   - 现在：`104` 笔，PF `0.87`
7. `BTCUSDT 1h 2022-01-24 ~ 2022-02-23`
   - 上一轮：`8` 笔
   - 现在：`8` 笔，PF `1.91`
...
1. 这轮依然不是“按某个周期特调”
   - `BTCUSDT 5m` 两段不同行情都一起抬频，说明修的是通用顺势恢复边界。
2. `15m / 1h` 基本稳定
   - 说明这轮没有把宽通道反转和失败突破链放空。
3. `楔形 / 头肩` 没有被一把放穿
   - `BTCUSDT 15m 2022` 里 `头肩` 仍是 `168 -> 150`，`楔形` 是 `41 -> 41`
   - `BTCUSDT 1h 2022` 里 `头肩` 仍是 `96 -> 72`，`楔形` 是 `21 -> 18`
   - 这说明 `second-leg trap` 还在工作，只是不再把所有宽通道反转一律压死。

## 二十一、主瓶颈已经转移

这轮之前，`5m` 的第一大问题还是顺势恢复链过早卡死：

- `宽通道顺势恢复缺少 follow-through`
- `深回调后的趋势延续质量不足`

这轮之后，这两条在 `BTCUSDT 5m` 的 top blocker 里已经明显退位，新的主瓶颈变成：

1. `目标路径在结构磁体受阻 (prior_level)`
   - `BTCUSDT 5m 2022`: `88`
   - `BTCUSDT 5m 2025`: `116`
2. `区间/弱趋势里的 H2/L2 仍缺少失败突破或趋势线破坏证据`
   - `BTCUSDT 5m 2022`: `48`
   - `BTCUSDT 5m 2025`: `59`
3. `宽通道逆势单仍需靠近边缘或优势区`
   - `BTCUSDT 5m 2022`: `106`
   - `BTCUSDT 5m 2025`: `143`
4. `宽通道反转更像 second-leg trap`
   - `BTCUSDT 1h 2022`: `15`
   - `BTCUSDT 15m 2025-11`: `16`
...
- “顺势恢复链已明显打开，但最后仍被 `prior_level`、`H2/L2` 反转证据和 `second-leg trap` 拦掉一批”

也就是说，下一轮如果继续审，最值的已经不是再碰 `follow-through`，而是：
