...
复测还是用同一组分散行情，不按单一品种或单一周期去改：

1. `BTCUSDT 5m 2022-01-24 ~ 2022-02-23`
   - 之前：`393` 笔，`13.1` 笔/天，PF `0.55`
   - 现在：`567` 笔，`18.9` 笔/天，PF `0.45`
2. `BTCUSDT 5m 2025-01-10 ~ 2025-02-09`
   - 之前：`388` 笔，`12.93` 笔/天，PF `0.43`
   - 现在：`520` 笔，`17.33` 笔/天，PF `0.43`
...
4. `ETHUSDT 15m 2023-07-10 ~ 2023-08-09`
   - 之前：`113` 笔
   - 现在：`113` 笔
5. `BNBUSDT 15m 2025-01-10 ~ 2025-02-09`
   - 之前：`107` 笔
   - 现在：`107` 笔
6. `SOLUSDT 15m 2025-01-10 ~ 2025-02-09`
   - 之前：`89` 笔
   - 现在：`91` 笔
7. `BTCUSDT 15m 2025-11-01 ~ 2025-11-30`
   - 之前：`103` 笔
   - 现在：`104` 笔
8. `BTCUSDT 1h 2022-01-24 ~ 2022-02-23`
   - 之前：`8` 笔
   - 现在：`8` 笔
...
1. 这轮放宽不是“按某个周期特调”
   - 因为它主要影响的是那些真实出现了 `H1/H2 + 通道恢复` 机会的样本。
   - `5m` 两段不同行情都一起抬频，说明不是单一窗口偶然。
2. `15m / 1h` 基本不动
   - 说明这轮改动没有去乱动失败突破反做链和宽通道反转主链。
   - 它只修复了顺势恢复链里本来写得过死的 veto。
...
- “顺势恢复链已打开，但很多机会最后被结构目标路径和反转证据要求挡住”
