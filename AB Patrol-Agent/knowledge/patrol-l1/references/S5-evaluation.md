...
- 本文件只负责：
  - `P / R / Trader's Equation`
  - `Scalp / Swing / 反转试探` 的风格判定
  - 风格、市场状态、订单类型的一致性检查
  - 执行前最后一层“这笔单值不值得做”的数学判断
- 本文件不负责：
  - 方向和 Daily 偏置 → 看 `S0/S2`
  - 市场状态与关键位 → 看 `S3/S3b`
  - 具体 playbook 的入场结构 → 看 `S4/S6-*`
  - 情绪、纪律、原话锚点 → 看 `Q3/Q4/Q5`
  - 仓位计算、接口、交易所实现 → 看运行说明和执行层
