...
> Patrol 运行时统一把 setup 分成 4 个阶段，避免“看到了结构”就误以为“已经能下单”。

| 阶段 | 含义 | 允许的事 |
|------|------|---------|
| **WATCH** | 只有背景，没有边缘优势或没有信号完成 | 记录观察，不生成委托 |
| **PRE_SIGNAL** | 有位置或反转线索，但还没完成升级条件 | 推送预信号，写 `planned_trade` 草案 |
| **CANDIDATE** | 已满足风格/位置/二次信号，但可能还没到精确价位 | 允许保留计划委托，继续等待触发 |
| **EXECUTABLE** | 结构、订单类型、trigger 价、TE 全部齐备 | 才允许进入真实动作链 |
...
1. **先位置，后信号**
   - 没到边缘或关键位，只能 `WATCH`
2. **先二次信号，后可执行**
   - 第一次反转通常只算 `PRE_SIGNAL`
3. **先计划委托，后真实委托**
   - 没有 `entry_price` 或明确 `entry_zone`，通常还只是 `CANDIDATE`
4. **订单类型由 PA 决定**
   - `TR` / 逆势 `BC` → `LIMIT`
   - 顺势 `BO/TC` → `STOP_MARKET`
