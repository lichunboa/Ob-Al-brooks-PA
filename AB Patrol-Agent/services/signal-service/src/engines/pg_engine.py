...
class PGSignalEngine(BaseEngine):
    ...
    def __init__(self, db_url: str = None, symbols: list[str] = None):
        ...
        self._last_stale_summary_at = 0.0
    ...
    def _wait_for_fresh_inputs(self, timeout: int = 90, poll_interval: int = 5) -> bool:
        """启动时等待一轮新鲜输入，避免 data-service 刚拉起时首轮全部判陈旧。"""
        ...
        while time.time() < deadline:
            ...
            for symbol in self.symbols:
                ...
                if curr_candle and self._is_fresh(curr_candle.get("bucket_ts"), "1m", 60):
                    fresh_candles += 1

                curr_metric = metrics.get(symbol)
                if curr_metric and self._is_fresh(curr_metric.get("create_time"), "5m", 300):
                    fresh_metrics += 1

            if fresh_candles == len(self.symbols):
                if attempts > 0:
                    logger.info(
                        "PG 输入已转新鲜: candles=%d/%d metrics=%d/%d",
                        fresh_candles,
                        len(self.symbols),
                        fresh_metrics,
                        len(self.symbols),
            ...
            if attempts == 0 or attempts % 3 == 0:
                logger.info(
                    "等待 PG 输入变新鲜... candles=%d/%d metrics=%d/%d",
                    fresh_candles,
                    len(self.symbols),
                    fresh_metrics,
                    len(self.symbols),
        ...
        logger.warning("等待 PG 输入超时，继续运行并使用当前可见数据")
        return False

    def _log_stale_summary(
        self,
        stale_candles: list[tuple[str, datetime | None]],
        stale_metrics: list[tuple[str, datetime | None]],
    ) -> None:
        """聚合陈旧输入告警，避免每轮每个 symbol 各打一条。"""
        now = time.time()
        if now - self._last_stale_summary_at < 55:
            return

        def _describe(items: list[tuple[str, datetime | None]], label: str) -> str:
            if not items:
                return ""
            latest = max((ts for _, ts in items if ts is not None), default=None)
            symbols = ", ".join(symbol for symbol, _ in items[:6])
            if len(items) > 6:
                symbols += ", ..."
            age = "-"
            if latest is not None:
                ref_now = datetime.now(latest.tzinfo) if latest.tzinfo else datetime.utcnow()
                age = f"{int((ref_now - latest).total_seconds())}s"
            return f"{label} {len(items)}个 [{symbols}] latest={latest} age={age}"

        parts = [
            _describe(stale_candles, "K线"),
            _describe(stale_metrics, "期货"),
        ]
        summary = " | ".join(part for part in parts if part)
        if summary:
            logger.warning("PG 输入陈旧，已跳过本轮部分检测: %s", summary)
            self._last_stale_summary_at = now
    ...
    def check_signals(self) -> list[PGSignal]:
        ...
        self._log_stale_summary(stale_candles, stale_metrics)
    ...
    def run_loop(self, interval: int = 60):
        ...
        self._wait_for_fresh_inputs(timeout=min(max(interval, 30), 90))
