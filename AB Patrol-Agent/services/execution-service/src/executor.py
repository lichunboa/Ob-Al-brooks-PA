...
class BinanceExecutor:
    ...
    def fetch_multi_tf_klines(self, symbol: str, limit: int = 150) -> dict:
        """多周期 K 线快照（5m/15m/1h 各 150 根，用于 PA 分析）

        Args:
            symbol: 交易对符号
            limit: 每个周期返回的 K 线数量（默认 150）
        """
        result = {}
        # patrol-l1 需要的 3 个核心周期
        for tf in ["5m", "15m", "1h"]:
            data = self.fetch_klines(symbol, tf, limit=limit)
            result[tf] = data

            # 检查返回的K线数量
            if "bars" in data:
                actual_count = len(data["bars"])
                if actual_count < limit * 0.9:  # 如果少于预期的90%，记录警告
                    logger.warning(
                        f"K线数量不足: {symbol} {tf} 预期{limit}根，实际{actual_count}根"
                    )
        return result
