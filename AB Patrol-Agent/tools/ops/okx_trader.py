...
Usage:
    uv run python tools/ops/okx_trader.py positions
    uv run python tools/ops/okx_trader.py balance
    uv run python tools/ops/okx_trader.py order BTCUSDT buy 10 125 --sl 60000 --tp 70000
    uv run python tools/ops/okx_trader.py close BTCUSDT
    uv run python tools/ops/okx_trader.py instrument BTCUSDT
    uv run python tools/ops/okx_trader.py calc-size BTCUSDT 67000 66500 45
    uv run python tools/ops/okx_trader.py init
"""
...
CONFIG_PATH = ROOT / "data" / "pa_trader" / "okx_config.json"
...
class OKXTrader:
    ...
    def get_klines(self, symbol, bar='5m', limit=30):
        ...
        for row in reversed(result['data']):  # OKX returns newest first
            ...
            time_str = datetime.fromtimestamp(ts/1000, tz=UTC).strftime('%H:%M')
            candles.append({'time': time_str, 'O': o, 'H': h, 'L': low_price, 'C': c, 'vol': vol})
        return candles
            ...
            json.dumps(
                {
                    'error': (
                        'Usage: okx_trader.py '
                        '<positions|balance|order|close|instrument|calc-size|modify-sl|init>'
                    )
                }
            )
        )
