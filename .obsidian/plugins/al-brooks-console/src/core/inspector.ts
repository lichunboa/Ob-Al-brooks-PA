...
export function buildInspectorIssues(
  ...
  for (const t of trades) {
    ...
    if (presets) {
      ...
      checkEnum(
        "account_type",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.accountType)?.value
      );
      checkEnum(
        "ticker",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.ticker)?.value
      );
      checkEnum(
        "timeframe",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.timeframe)?.value
      );
      checkEnum(
        "direction",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.direction)?.value
      );
      checkEnum(
        "market_cycle",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.marketCycle)?.value
      );
      checkEnum(
        "setup_category",
      ...
      checkEnum(
        "patterns_observed",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.patternsObserved)?.value
      );
      checkEnum(
        "probability",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.probability)?.value
      );
      checkEnum(
        "outcome",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.outcome)?.value
      );
      checkEnum(
        "execution_quality",
        getFirstFieldValue(fm, TRADE_FIELD_ALIASES.executionQuality)?.value
      );
      checkEnum(
        "strategy_name",
  ...
  for (const t of trades) {
    ...
    const normalizeEnumField = (
      ...
      if (isMulti) {
        ...
        const changed = rawValues.some(
          (v, i) => (presets.normalize(fieldKey, v) ?? v) !== rawValues[i]
        );
    ...
    normalizeEnumField(
      "setup_category",
      TRADE_FIELD_ALIASES.setupCategory,
      false
    );
    ...
    normalizeEnumField(
      "execution_quality",
      TRADE_FIELD_ALIASES.executionQuality,
      false
    );
    normalizeEnumField(
      "strategy_name",
      TRADE_FIELD_ALIASES.strategyName,
      false
    );
    ...
    if (Object.keys(updates).length > 0)
      fileUpdates.push({ path: t.path, updates });
  }
