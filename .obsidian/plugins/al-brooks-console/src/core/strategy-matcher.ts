...
export function matchStrategies(index: StrategyIndex, input: StrategyMatchInput): StrategyCard[] {
	...
	for (const card of index.list()) {
		...
		const setupHit =
			wantSetupKey &&
			card.setupCategories.some((c) => normalizeKey(c).includes(wantSetupKey) || wantSetupKey.includes(normalizeKey(c)));
		const cycleHit =
			wantCycleKey &&
			card.marketCycles.some((c) => normalizeKey(c).includes(wantCycleKey) || wantCycleKey.includes(normalizeKey(c)));
