...
import { FIELD_ALIASES, normalizeAccountType, normalizeOutcome, parseNumber } from "./field-mapper";
...
function getFirstHit(frontmatter: Record<string, any>, keys: readonly string[]) {
	for (const k of keys) {
		if (frontmatter?.[k] !== undefined) return { key: k, value: frontmatter[k] };
	}
	return undefined;
}
...
export function buildTradeNormalizationPlan(
	...
	for (const t of trades) {
		...
		const moveFirst = (aliases: readonly string[], canonicalKey: string) => {
			...
			if (options.includeDeleteKeys && hit.key !== canonicalKey) deleteKeys.push(hit.key);
		};
		...
		const acctRaw = fm[canonicalKeys.accountType] ?? getFirstHit(fm, FIELD_ALIASES.accountType)?.value;
		const acctStr = asNonEmptyString(acctRaw);
		if (acctStr) {
			const preset = presets?.normalize("account_type", acctStr);
			const normalized = preset ?? (normalizeAccountType(acctStr) ? preset ?? acctStr : acctStr);
			if (preset && preset !== acctStr) updates[canonicalKeys.accountType] = preset;
		}

		const outcomeRaw = fm[canonicalKeys.outcome] ?? getFirstHit(fm, FIELD_ALIASES.outcome)?.value;
		const outcomeStr = asNonEmptyString(outcomeRaw);
		if (outcomeStr) {
			const preset = presets?.normalize("outcome", outcomeStr);
			if (preset && preset !== outcomeStr) updates[canonicalKeys.outcome] = preset;
			else {
		...
		const pnlRaw = fm[canonicalKeys.pnl] ?? getFirstHit(fm, FIELD_ALIASES.pnl)?.value;
		...
		if (presets) {
			const normalizeEnum = (fieldKey: string, canonicalKey: string, isMulti: boolean) => {
		...
		if (Object.keys(updates).length > 0 || deleteKeys.length > 0) {
			fileUpdates.push({ path: t.path, updates, deleteKeys: deleteKeys.length ? deleteKeys : undefined });
		}
	}
...
export function buildStrategyMaintenancePlan(
	...
	for (const n of notes) {
		...
		// Minimal defaults
		if (fm[canonicalKeys.strategyName] === undefined && fm["strategy_name"] !== undefined) {
		...
		if (fm[canonicalKeys.marketCycle] === undefined) updates[canonicalKeys.marketCycle] = Array.isArray(fm[canonicalKeys.marketCycle]) ? fm[canonicalKeys.marketCycle] : [];
		if (fm[canonicalKeys.patternsObserved] === undefined) updates[canonicalKeys.patternsObserved] = Array.isArray(fm[canonicalKeys.patternsObserved]) ? fm[canonicalKeys.patternsObserved] : [];
		...
		if (Object.keys(updates).length > 0) fileUpdates.push({ path: n.path, updates });
	}
