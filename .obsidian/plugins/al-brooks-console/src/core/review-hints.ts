...
import { FIELD_ALIASES, getFirstFieldValue, normalizeTicker, parseNumber } from "./field-mapper";

const REVIEW_HINT_FIELD_ALIASES = {
	setup: ["setup", "setupKey", "setup_key", "设置/setup", "设置", "形态/setup", "形态"],
	...
	strategyName: ["strategyName", "strategy", "策略/strategyName", "策略名称/strategyName", "策略名称"],
	error: [
...
function getString(fm: Record<string, any> | undefined, keys: readonly string[]): string | undefined {
...
function getStringArray(fm: Record<string, any> | undefined, keys: readonly string[]): string[] {
	if (!fm) return [];
	const v = getFirstFieldValue(fm, keys);
...
function getTicker(trade: TradeRecord): string | undefined {
	if (typeof trade.ticker === "string" && trade.ticker.trim().length) return trade.ticker;
...
export function buildReviewHints(trade: TradeRecord): ReviewHint[] {
	try {
		...
		const push = (id: string, zh: string, en: string) => hints.push({ id, zh, en });
		...
		if (hasValue(err) && String(err).trim() !== "None" && String(err).trim() !== "无") {
			push(
				"error_review",
