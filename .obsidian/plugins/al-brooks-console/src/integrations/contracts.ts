...
export function listCommands(app: App): CommandInfo[] {
	...
	if (typeof commandsAny?.listCommands === "function") {
		try {
			const list = commandsAny.listCommands() as Array<{ id: string; name?: string }>;
	...
	const dict = commandsAny?.commands as Record<string, { id: string; name?: string }> | undefined;
...
export function findCommandByPrefix(app: App, prefix: string): CommandInfo | undefined {
