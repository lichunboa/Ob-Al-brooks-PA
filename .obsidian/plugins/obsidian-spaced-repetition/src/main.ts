...
export default class SRPlugin extends Plugin {
    ...
    /**
     * 通过文件路径打开卡片复习（供 Al Brooks Console 调用）
     * @param filePath - 文件路径
     * @param mode - 复习模式（0=Review, 1=Cram）
     * @returns 是否成功
     */
    public async reviewByFilePath(filePath: string, mode: number = 0): Promise<boolean> {
        const file = this.app.vault.getAbstractFileByPath(filePath);
        if (!(file instanceof TFile)) {
            console.error("[SRS] File not found:", filePath);
            return false;
        }

        try {
            const reviewMode = mode === 1 ? FlashcardReviewMode.Cram : FlashcardReviewMode.Review;

            if (this.data.settings.openViewInNewTab) {
                this.tabViewManager.openSRTabView(this.osrAppCore, reviewMode, file);
            } else {
                await this.openFlashcardModalForSingleNote(file, reviewMode);
            }
            return true;
        } catch (e) {
            console.error("[SRS] Failed to open review:", e);
            return false;
        }
    }
