...
def _find_project_root():
    """查找项目根目录"""
    possible_roots = [
        Path(__file__).parents[2],  # 容器: /app
        Path(__file__).parents[3],  # 本地开发
        Path(__file__).parents[4],  # TradeCat 原始
        Path("/app"),               # 容器绝对路径
    ]
    for root in possible_roots:
        if (root / "config" / ".env").exists() or (root / ".env").exists():
            return root
    return Path("/app")  # 默认容器路径

_PROJECT_ROOT = _find_project_root()
# .env 可能在 config/.env 或根目录 .env
ENV_PATH = _PROJECT_ROOT / "config" / ".env" if (_PROJECT_ROOT / "config" / ".env").exists() else _PROJECT_ROOT / ".env"
