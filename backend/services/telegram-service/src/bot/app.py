...
# ================== 币种列表全局缓存 ==================
# 用于缓存从交易所获取的币种列表，避免频繁API调用
_SYMBOLS_CACHE: Optional[List[str]] = None
_SYMBOLS_CACHE_TIME: float = 0
_SYMBOLS_CACHE_TTL: int = 3600  # 1小时缓存，避免429错误
...
# Binance API - 从环境变量读取，默认启用
BINANCE_API_DISABLED = os.environ.get('BINANCE_API_DISABLED', '0') == '1'
...
class TradeCatBot:
    ...
    def get_active_symbols(self, force_refresh=False):
        ...
        优先使用 SYMBOLS_GROUPS 配置。auto模式下使用1小时缓存避免API频率限制。
        """
        ...
        # auto/all 模式: 从交易所获取全部 USDT 永续合约（使用全局缓存）
        groups = os.environ.get("SYMBOLS_GROUPS", "main4").lower()
        if groups in ("auto", "all"):
            ...
            with _SYMBOLS_CACHE_LOCK:
                if not force_refresh and _SYMBOLS_CACHE and (now - _SYMBOLS_CACHE_TIME) < _SYMBOLS_CACHE_TTL:
                    logger.debug("✅ 使用币种列表缓存 (%d 个币种, 缓存剩余 %d 秒)",
                                len(_SYMBOLS_CACHE), _SYMBOLS_CACHE_TTL - (now - _SYMBOLS_CACHE_TIME))
                    filtered = [s for s in _SYMBOLS_CACHE if s not in get_blocked_symbols()]
                    self._active_symbols = filtered
                    self._active_symbols_timestamp = now
                    return filtered
            ...
            try:
                ...
                # 应用排除
                exclude_str = os.environ.get("SYMBOLS_EXCLUDE", "")
                exclude = set(s.strip().upper() for s in exclude_str.split(",") if s.strip())
                filtered = [s for s in all_symbols if s not in exclude and s not in get_blocked_symbols()]
                ...
                self._active_symbols = filtered
                self._active_symbols_timestamp = now
                logger.info("✅ 从交易所获取币种 (auto模式): %d 个币种, 已缓存1小时", len(filtered))
                return filtered
            except Exception as exc:
                logger.error("❌ 从交易所获取币种失败: %s, 回退到 main4", exc)
                # 如果有过期缓存，即使出错也使用
                with _SYMBOLS_CACHE_LOCK:
                    if _SYMBOLS_CACHE:
                        logger.warning("⚠️ 使用过期缓存 (%d 个币种)", len(_SYMBOLS_CACHE))
                        filtered = [s for s in _SYMBOLS_CACHE if s not in get_blocked_symbols()]
                        self._active_symbols = filtered
                        self._active_symbols_timestamp = now
                        return filtered
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if button_data == "start_ai_analysis":
        logger.info("=== AI分析按钮被点击 ===")
        try:
            logger.info("尝试导入 ai_integration 模块...")
            from bot.ai_integration import get_ai_handler, AI_SERVICE_AVAILABLE, SELECTING_COIN
            logger.info(f"✅ 导入成功! AI_SERVICE_AVAILABLE = {AI_SERVICE_AVAILABLE}")

            if not AI_SERVICE_AVAILABLE:
                logger.warning("❌ AI_SERVICE_AVAILABLE = False, 抛出ImportError")
                raise ImportError("ai-service 未安装")

            logger.info("创建 AI handler...")
            ai_handler = get_ai_handler(symbols_provider=lambda: user_handler.get_active_symbols() if user_handler else None)
            logger.info(f"✅ AI handler 创建成功: {ai_handler}")

            context.user_data["ai_state"] = SELECTING_COIN
            logger.info("调用 start_ai_analysis...")
            await ai_handler.start_ai_analysis(update, context)
            logger.info("✅ AI分析启动成功")
            return
        except ImportError as e:
            logger.warning(f"❌ AI模块导入失败 (ImportError): {e}")
            import traceback
            ...
            logger.error(f"❌ AI分析启动失败 (Exception): {e}")
            import traceback
    ...
    if button_data == "start_coin_analysis":
        logger.info("=== 🤖 AI分析按钮被点击 (start_coin_analysis) ===")
        try:
            logger.info("尝试导入 bot.ai_integration...")
            from bot.ai_integration import get_ai_handler, AI_SERVICE_AVAILABLE
            logger.info(f"✅ 导入成功! AI_SERVICE_AVAILABLE = {AI_SERVICE_AVAILABLE}")

            if not AI_SERVICE_AVAILABLE:
                logger.warning("❌ AI_SERVICE_AVAILABLE = False, 显示未安装消息")
                await query.answer(_t(update, "ai.not_installed"), show_alert=True)
                return

            logger.info("AI服务可用，创建handler...")
            context.user_data["lang_preference"] = _resolve_lang(update)
            ai_handler = get_ai_handler(symbols_provider=lambda: user_handler.get_active_symbols() if user_handler else None)
            logger.info(f"✅ Handler创建成功: {ai_handler}")
            ...
            logger.error(f"❌ AI模块导入失败 (ImportError): {e}")
            import traceback
            logger.error(f"Traceback:\n{traceback.format_exc()}")
            await query.answer(_t(update, "ai.failed", error=str(e)), show_alert=True)
            return
        except Exception as e:
            logger.error(f"❌ AI分析入口失败 (Exception): {e}")
            import traceback
            logger.error(f"Traceback:\n{traceback.format_exc()}")
