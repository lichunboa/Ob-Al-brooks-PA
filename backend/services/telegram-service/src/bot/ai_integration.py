...
# 添加 ai-service 到 path (尝试多个可能的位置)
_possible_ai_paths = [
    Path(__file__).resolve().parents[2] / "ai-service",  # 容器
    Path(__file__).resolve().parents[3] / "ai-service",  # 本地
    Path("/app/ai-service"),  # 容器绝对路径
]
for _path in _possible_ai_paths:
...
_possible_roots = [
    _file_path.parents[2] if len(_file_path.parents) > 2 else None,  # 容器: /app
    _file_path.parents[3] if len(_file_path.parents) > 3 else None,
    _file_path.parents[4] if len(_file_path.parents) > 4 else None,
    Path("/app"),
]
for PROJECT_ROOT in _possible_roots:
