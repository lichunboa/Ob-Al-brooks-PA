...
# 在容器中 libs 在 /app/libs，尝试多个可能的路径
def _find_libs_path():
    """安全地查找 libs 路径，避免 IndexError"""
    _current = _Path(__file__).resolve()
    _parents = list(_current.parents)

    # 尝试不同层级
    for i in range(min(5, len(_parents))):
        candidate = _parents[i] / "libs"
        if candidate.exists():
            return candidate

    # 回退到容器绝对路径
    if _Path("/app/libs").exists():
        return _Path("/app/libs")

    return None

_libs_path = _find_libs_path()
if _libs_path and str(_libs_path) not in _sys.path:
    _sys.path.insert(0, str(_libs_path))
...
# ============================================================
# RankingDataProvider（indicators.db / market_data.db）
# ============================================================
import os as _os
...
class RankingDataProvider:
    def __init__(self, db_path: Optional[Path] = None) -> None:
        # 优先使用环境变量指定的路径
        env_db_path = _os.environ.get("INDICATOR_SQLITE_PATH")
        if env_db_path:
            _default_db = Path(env_db_path)
        else:
            # 回退到原始路径
            ...
            # 尝试其他可能的位置
            alt_paths = [
                Path("/app/data/indicators.db"),  # 容器内
                _project_root / "data" / "indicators.db",  # 本地开发
            ]
        ...
        LOGGER.info("使用数据库: %s (存在: %s)", self.db_path, self.db_path.exists())
