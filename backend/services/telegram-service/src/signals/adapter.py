...
# 添加 signal-service 到路径 (尝试多个可能的位置)
_possible_signal_paths = [
    Path(__file__).resolve().parents[2] / "services" / "signal-service" / "src",  # 容器内
    Path(__file__).resolve().parents[3] / "services" / "signal-service" / "src",  # 本地
    Path(__file__).resolve().parents[4] / "services" / "signal-service" / "src",  # TradeCat 原始
    Path("/app/signal-service/src"),  # 容器绝对路径
]
...
try:
    ...
    logger.warning(f"signal-service 模块不可用: {e}")
