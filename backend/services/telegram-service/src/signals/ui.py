...
# 从 signal-service 导入 (尝试多个可能的位置)
_possible_signal_paths = [
    Path(__file__).resolve().parents[2] / "services" / "signal-service" / "src",
    Path(__file__).resolve().parents[3] / "services" / "signal-service" / "src",
    Path(__file__).resolve().parents[4] / "services" / "signal-service" / "src",
    Path("/app/signal-service/src"),
]
...
# 导入 signal-service 模块 (容错处理)
try:
    from rules import RULES_BY_TABLE
    from storage.history import SignalHistory, get_history
    _SIGNAL_UI_AVAILABLE = True
except ImportError as e:
    logging.getLogger(__name__).warning(f"signal-service UI 模块不可用: {e}")
    _SIGNAL_UI_AVAILABLE = False
    RULES_BY_TABLE = {}
    class SignalHistory: pass
    def get_history(*args, **kwargs): return None
